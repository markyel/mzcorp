...
<div class="max-w-[1320px] mx-auto px-6 pt-3 pb-8"
    ...
    <div class="ds-card p-[18px] mb-4 grid gap-4" style="grid-template-columns: 1fr auto">
        ...
        <div class="flex flex-col gap-2 min-w-[200px]">
            ...
            {{-- Модуль поставщиков: пометить пойманный тред как наш запрос
                 поставщику. Заявка-фантом (создана из ответа поставщика на наш
                 запрос расценки) закрывается, переписка уходит в раздел
                 «Поставщики», будущие ответы в этом треде не плодят заявок.
                 Owner/acting/privileged, только для активных заявок. --}}
            @if(($canManage || $canReassign) && ! $req->status->isTerminal() && $req->status !== $RS::Pending)
                <button type="button"
                        wire:click="markAsSupplierInquiry"
                        wire:confirm="Это не заявка клиента, а ответ поставщика на наш запрос расценки? Заявка будет закрыта, переписка уйдёт в раздел «Поставщики»."
                        class="btn btn-sm">📦 Это запрос поставщику</button>
            @endif
