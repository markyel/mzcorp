...
<div wire:key="position-card-{{ $item->id }}"
    ...
    <div class="grid items-center px-[12px] {{ $isExpanded ? 'py-2 border-b border-border-subtle' : 'py-1' }} gap-2.5 min-h-[44px]"
        ...
        {{-- Qty --}}
        <div class="text-right mono text-[13px] text-fg-1 whitespace-nowrap">
            {{ rtrim(rtrim((string) $item->parsed_qty, '0'), '.') ?: '—' }} {{ $item->parsed_unit }}
            @if($isMeasured)
                <span class="block text-fg-3 text-[10.5px]" title="вторая размерность из заявки">× {{ rtrim(rtrim(number_format((float) $item->parsed_length, 3, '.', ''), '0'), '.') }} {{ $item->parsed_length_unit }}</span>
            @endif
        </div>

        {{-- Price --}}
        <div class="text-right mono text-[12px] whitespace-nowrap {{ $price !== null ? 'text-fg-1' : 'text-fg-3' }}"
             title="{{ $ci ? 'из каталога, обновлено ' . ($ci->last_imported_at?->format('d.m.Y') ?? '—') . ($ci->is_price_actual === false ? ' · ⚠ цена не актуальна' : '') : '' }}">
        ...
        <div class="text-right text-[11.5px] {{ $stock === null ? 'text-fg-3' : ($stock > 0 ? 'text-emerald-700' : 'text-amber-700') }}">
            ...
            @else
                ...
                @if($ci?->lead_time_days !== null)
                    <small class="block text-fg-3 text-[10px] mt-0.5"
            ...
            @if(!empty($ci?->stock_in_transit))
                ...
                <small class="block text-sky-700 text-[10px] mt-0.5"
