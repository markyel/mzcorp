    ...
    @if($open)
        <div style="position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.55); display: flex; align-items: flex-start; justify-content: center; padding: 60px 24px 24px;"
            ...
            <div class="ds-card p-5 w-full {{ $comparing ? 'max-w-[1200px]' : 'max-w-[1040px]' }} max-h-[90vh] flex flex-col" wire:click.stop>
                ...
                @endif
                @else
                    {{-- ─────────── Compare-панель (row-based stack) ───────────
                         Каждая позиция (subject + N catalogs) — отдельная
                         горизонтальная строка: фото слева 140px, описание
                         справа. Прокрутка вертикальная.

                         Раньше был grid с N+1 колонками (subject + кандидаты)
                         и фото на всю ширину колонки. При 1-2 кандидатах
                         колонка получалась 500-600px, фото 500x500 уходило
                         за пределы модала и body-scroll отрабатывал вместо
                         modal-scroll. --}}
                    @php
                    ...
                    <div class="flex-1 overflow-y-auto">
                        <div class="flex flex-col gap-3">
                            {{-- ── Subject row (позиция заявки) ── --}}
                            <div class="border border-sky-300 rounded-md bg-sky-50/40 p-3 flex gap-3 items-start">
                                {{-- Photo column (фиксированная ширина 140px) --}}
                                                ...
                                                <template x-if="items.length > 1">
                                                    <div>
                                                        <button type="button"
                                                                ...
                                                                style="position: absolute; left: 4px; top: 50%; transform: translateY(-50%); width: 24px; height: 24px; border-radius: 50%; border: none; background: rgba(0,0,0,0.55); color: white; font-size: 14px; cursor: pointer;">‹</button>
                                                        <button type="button"
                                                                x-on:click.stop="idx = (idx + 1) % items.length"
                                                                title="Следующее фото"
                                                                style="position: absolute; right: 4px; top: 50%; transform: translateY(-50%); width: 24px; height: 24px; border-radius: 50%; border: none; background: rgba(0,0,0,0.55); color: white; font-size: 14px; cursor: pointer;">›</button>
                                                        <div style="position: absolute; bottom: 4px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.55); color: white; padding: 1px 5px; border-radius: 6px; font-size: 9px; font-family: var(--font-mono);">
                                            ...
                                            <div class="border-t border-border-subtle pt-1.5 mt-1 text-[11px]">
                                                ...
                                                <div class="flex flex-wrap gap-x-3 gap-y-0.5">
                                                    @foreach($subjExtracted as $slug => $value)
                                                        <span class="mono"><span class="text-fg-3">{{ $slug }}:</span> <span class="text-fg-1">{{ is_scalar($value) ? $value : json_encode($value, JSON_UNESCAPED_UNICODE) }}</span></span>
                                                    @endforeach
                                                </div>
                                            </div>
                                        @endif
                            ...
                            {{-- ── Catalog rows ── --}}
                            @foreach($cmp as $c)
                                <div class="border border-border rounded-md bg-surface p-3 flex gap-3 items-start {{ $selectedCatalogId === $c->id ? 'ring-2 ring-emerald-400' : '' }}">
                                    {{-- Photo column (фиксированная ширина 140px) --}}
                                    <div class="shrink-0 w-[140px]">
                                        @if($c->photo_url)
                                    ...
                                    <div class="flex-1 min-w-0 flex flex-col gap-1.5">
                                        ...
                                        <div class="pt-1.5 mt-1 border-t border-border-subtle flex gap-1.5">
                                            <button type="button" wire:click="selectCatalog({{ $c->id }})"
                                                    class="btn btn-sm {{ $selectedCatalogId === $c->id ? 'btn-primary' : '' }}">
                                                {{ $selectedCatalogId === $c->id ? '✓ Выбрано' : 'Выбрать' }}
                                            </button>
                                            <a href="https://mylift.ru/?text={{ urlencode($c->sku) }}&fn=find"
                                               target="_blank" rel="noopener noreferrer"
                                               class="btn btn-sm" title="Открыть на mylift.ru">↗ mylift</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
