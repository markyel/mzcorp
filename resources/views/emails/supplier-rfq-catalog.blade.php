...
<body style="margin:0;padding:0;font-family:-apple-system,Arial,sans-serif;background:#f5f6f8;color:#1a1a1a">
    <div style="max-width:640px;margin:0 auto;padding:20px 14px">
        ...
        <p style="font-size:14px;margin:0 0 16px;white-space:pre-line">@if(trim((string) ($intro ?? '')) !== ''){{ $intro }}@else{{ $t['intro_a'] }} <b>{{ $t['intro_b'] }}</b> {{ $t['intro_c'] }}@endif</p>
        ...
        <p style="font-size:13px;color:#4b5563;margin:0 0 4px;white-space:pre-line">{{ trim((string) ($closing ?? '')) !== '' ? $closing : $t['reply'] }}</p>
    </div>
</body>
</html>
