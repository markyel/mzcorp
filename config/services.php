...
return [
    ...
    */
    'inheritance' => [
        ...
        // Гибрид: до скольких дней после «тихого» закрытия (нет ответа)
        // РЕАНИМИРУЕМ СТАРУЮ заявку (переоткрываем под тем же номером) вместо
        // создания связанной новой — но только если позиции РОВНО те же
        // (все совпали по артикулу). Старше окна или иные позиции → наследование.
        'reanimate_max_days' => (int) env('INHERITANCE_REANIMATE_MAX_DAYS', 45),
        // Гард постпродажи: ответ в закрытый тред про документооборот/отгрузку
        // (реквизиты, УПД, закрывающие, комплектация) при наличии у клиента
        // оплаченного/выигранного заказа не старше N дней → переписка уходит
        // на заказ (post_sale), заявка-наследник не создаётся. Кейс M-2026-7877.
        'post_sale_reroute_max_days' => (int) env('INHERITANCE_POST_SALE_REROUTE_MAX_DAYS', 60),
    ],
    ...
    */
    'catalog_name_match' => [
        ...
        // Подавать ли авто-категорию из KB в промпт LLM-реранка. На части
        ...
        // Жёсткий пре-фильтр по бренду (isBrandSafe отбрасывает кандидата при
        // несовпадении брендов). Переусердствует на мис-парсе/дистрибьюторских
    ...
    */
    ...
    // YML-фид «Цены и наличие» для площадки LazyLift/Liftway (MyLift-поставщик).
    // Цена фида = purchase_price × markup. Дефолт 1.15 (закупка + 15%).
    'liftway_feed' => [
        'markup' => (float) env('LIFTWAY_FEED_MARKUP', 1.15),
        // Лимит позиций в ТЕСТОВОМ фиде полной карточки (full.yml). 0 = без лимита.
        'full_limit' => (int) env('LIFTWAY_FEED_FULL_LIMIT', 100),
    ...
    */
    'assignment' => [
        ...
        // Окно для sticky по НОРМАЛИЗОВАННЫМ артикулам (Level 3, ItemTokenizer):
        // парная заявка на ту же позицию приходит в пределах нескольких дней
        // (клиент напрямую + та же заявка с площадки). Открытая заявка
        // двухмесячной давности новую к себе притягивать не должна — иначе
        // популярная позиция навсегда закрепляется за одним менеджером.
        'text_sticky_window_days' => (int) env('ASSIGNMENT_TEXT_STICKY_WINDOW_DAYS', 30),
    ...
    'dealer' => [
        ...
        // пришло столько заявок или больше за окно `auto_window_days` (любой
        // статус) — он автоматически помечается и client-sticky (1b) для него
        // отключается. Catalog/text-sticky продолжают работать. 0 — выключить
        // автопометку. Окно вместо «N одновременно открытых» ловит и
        // дистрибьюторов с быстрым оборотом (быстро закрывают сделки, поэтому
        // одновременно открытых мало, но общий поток большой).
        'auto_threshold' => (int) env('DEALER_AUTO_THRESHOLD', 8),
        // Скользящее окно в днях для подсчёта потока заявок. 0/отрицательное —
        // без окна (по всей истории).
        'auto_window_days' => (int) env('DEALER_AUTO_WINDOW_DAYS', 30),
    ],
    ...
    'company' => [
        ...
        'inn' => '7715802492',
        /*
         * ВСЕ наши ИНН, от которых мы продаём. Разбор реквизитов покупателя
         * считает покупателем любой ИНН, кроме наших, — и счёт физлицу, где
         * продавцом стоит наше второе юрлицо (ИП Роденков), заводил это
         * юрлицо в реестр клиентов и цеплял к адресам заказчиков.
         * Задаётся через COMPANY_OWN_INNS (через запятую).
         */
        'own_inns' => array_values(array_filter(array_map(
            fn ($v) => preg_replace('/\D+/', '', trim((string) $v)),
            explode(',', (string) env('COMPANY_OWN_INNS', '7715802492,771519349487')),
        ))),
        'kpp' => '770101001',
        'postal_code' => '105082',
        ...
        // Оператор ЭДО + идентификатор участника. Показывается в email-подписи
        // менеджеров (EmailSignatureService). Сменился 2026-08-05: Диадок →
        // Калуга.Астрал. Название оператора вынесено в edo_operator, чтобы
        // меняться в одном месте (не хардкодом в сервисе).
        'edo_operator' => 'Калуга.Астрал',
        'edo_id' => '2AEDA49A6C3-1F70-4E9B-8F17-2177F3F8BE7F',
