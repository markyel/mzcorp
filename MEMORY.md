...
### Открыто (2026-09-08)
- ⏳ **Папки почты живут только в mzCorp** — письма физически остаются в INBOX Яндекса (UID MOVE на Yandex не чистит оригинал без EXPUNGE, см. «Известные грабли»). Если заказчик захочет видеть папки и в Яндексе — нужен отдельный синк папок (COPY + \Seen оригинала, как в `MailFolderRouter`).
- ⏳ **Pull \Seen только для INBOX за 14 дней** (`ImapSeenSyncService::PULL_DAYS`) и только по личным ящикам с владельцем; общие ящики (info@ и т.п.) двусторонне не синкаются — у них нет «владельца», прочитанность в них per-user.
