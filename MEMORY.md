...
### ▶ Сессия 2026-09-25 — итог (ТОЧКА ВХОДА для следующей сессии)

Всё на проде, HEAD `c192fe4` (+ этот коммит памяти). Три блока: реестр клиентов, детекторы почты, полное зеркало почтовых ящиков.

**1. Зеркало истории личных ящиков — ИДЁТ ИМПОРТ, проверить первым делом.** Заказчик: «полная синхронизация ящиков менеджеров, в т.ч. папок». Решение заказчика: письма истории заводятся **шапкой** (отправитель, получатели, тема, дата, флаги), тело и вложения — **с Яндекса при первом открытии**; охват — `Mailbox::syncable()` личные ящики (менеджеры, РОП, закупки); ящик директора (Боев) и общие (info@, rfq@) НЕ зеркалятся. Полный импорт тел не лезет: 1,05 млн писем ≈ 150 ГБ БД + 300 ГБ файлов при 31 ГБ свободных на VPS.
- Код: `MailHistoryMirrorService` + `mail:history-mirror {--mailbox=*} {--only=inbox|sent|folders} {--shard=k/n} {--budget} {--seconds}`; строки `email_messages.is_history=true` (+ `body_fetched_at`, `history_has_attachments`), прогресс в `mailbox_folder_states.history_low_uid / history_completed_at / history_imported`. INBOX/Sent — только UID ≤ водяного знака живого синка; пользовательские папки — целиком. По расписанию каждые 10 мин `--seconds=120 --budget=3000` (поддержка: письма, которые правила Яндекса кладут в папки мимо INBOX).
- **Глобальный scope `ExcludeMailHistoryScope`** на `EmailMessage`: историю видят ТОЛЬКО почтовый клиент и синк через `EmailMessage::withHistory()` (Client, Composer, ImapFolderSyncService, ImapSeenSyncService, MailboxFolderService, MessagePersister, `SharedMailService::threadFor(..., withHistory: true)`, route binding, `EmailAttachment::emailMessage`). Конвейер, заявки, детекторы, отчёты, аналитика историю не видят. Сырой SQL scope обходит — `WeeklyManagerReportService` фильтрует `is_history=false` явно; **любой новый сырой SQL по email_messages, считающий письма, обязан фильтровать is_history**. «Это заявка!» на архивном письме сначала качает тело и снимает `is_history`.
- Тело: `MailHistoryMirrorService::fetchBody` (FT_PEEK — webklex сам снимает \Seen у непрочитанного, `Message::peek()`), автоматически при открытии, кнопка «Загрузить текст» для писем треда, ответ/пересылка качают сами. Проверено: письмо с 9 xlsx за 7 с, флаги сервера не изменились.
- **✅ ИМПОРТ ЗАВЕРШЁН 26.09 ~03:00:** сверка «сервер Яндекса ↔ у нас» по 9 ящикам — 1 115 664 из 1 117 332 (**99,9%**), история 1 017 832 строки. Остаток: Курзаев Sent 423 (Яндекс не отдал — доберёт плановый прогон), у остальных разница 0–84 — **дубли одного письма в одной папке** (у Яндекса две копии с разными UID, у нас уникальность mailbox+folder+message_id → одна строка), у Милючихина таких 793. Ещё две правки после записи ниже: повторы пачек мельче (500→100→50→25 — медленный ящик Курзаева упирался в таймаут чтения, webklex молча обрезал ответ) и **письмо в INBOX и Sent одновременно (себе/копия себе) → отдельная строка на каждую папку** (у Курзаева ~34 тыс. таких пропускалось). Сверку повторить: `/tmp/recon.php` на проде (если нет — LIST → `examineFolder()['exists']` vs `EmailMessage::withHistory()->where(mailbox, folder)->whereNotNull('imap_uid')->count()`).
- (История прогона) **Состояние на 22:30 25.09:** заведено 890 976 писем истории (mb7 Курзаев 204k, mb14 Якубович 178k, mb12 Румянцев 148k, mb8 Васюхно 112k, mb13 Агрызков 96k, mb11 Ежов 74k, mb19 Петрова 48k, mb18 Милючихин 29k, mb9 Головнёв 0,6k). Идёт добирающий проход **только по Курзаеву** (`/tmp/mirror_run5.sh`, лог `storage/logs/history-mirror/pass5_mb7.log`, завершение — строка `ALL DONE` в `_done5.log`), INBOX 40/75k, потом Sent. **Если процесс умер** — перезапустить `cd /var/www/mzcorp && sudo -u www-data php artisan mail:history-mirror --mailbox=7` (строго ОДНО соединение на ящик: при 2+ Яндекс режет FETCH). Затем по каждому ящику один обычный прогон без шарда, чтобы проставились `history_completed_at`, и сверка «сервер vs БД» по папкам (скрипт: для каждого ящика LIST → `examineFolder()['exists']` vs `EmailMessage::withHistory()->where(mailbox, folder)->count()`).
- Грабли, найденные по дороге: (а) под нагрузкой Яндекс отвечает пустым/обрезанным FETCH — повторы 4 раза с отступом 5–10–20 с + переподключение; (б) **webklex HeaderDecoder** на имени адресата с «\» зовёт `property_exists('')` → предупреждение Composer «Uninitialized string offset 0» → Laravel делает исключение → падала вся пачка 500 писем (десятки тысяч «недоданных»). Лечение: разбор шапки в `quietly()` + `fallbackRow()` для неразборчивых; (в) `ImapProtocol::getFlags()` не существует (есть `flags()`) — в `SyncMailboxFolderJob` pre-flight молча падает в catch, безвредно, т.к. webklex FT_PEEK сам возвращает \Seen; (г) **IPv6 до imap.yandex.ru нет маршрута** на VPS — изредка «Network is unreachable»; выдан чип-задача «Force IPv4 for Yandex IMAP» (правка /etc/gai.conf, ждёт решения заказчика); (д) `pkill -f` с шаблоном в тексте ssh-команды убивает собственную сессию — класть команды в файл.
- Побочно: у Агрызкова 6 тыс. писем «Входящих (локально)» с мая были в БД, но скрыты (потерян UID после остановленного 08.09 backfill) — зеркало вернуло связь (`uid_filled`). Индексы: `email_attachments(email_message_id)` (список почты 600→40 мс), `email_messages(message_id)`, `(in_reply_to)`, GIN `references_header`, `(mailbox_id, lower(message_id))`.
- Предупредить менеджеров: бейджи непрочитанных теперь честные, как в Яндексе.

**2. Реестр клиентов (раздел «Клиенты»).**
- DaData: ключ **только в прод `.env` `DADATA_API_KEY`** (в git не коммитить, не выводить). `OrganizationRegistryService`: РФ по ИНН (`findById/party`), **Беларусь по УНП 9 цифр** (`party_by`). Ночью `clients:registry-sync --limit=1500` 05:00. Юр.адрес карточки юрлица = реестр (пока не правил человек; прежний адрес из документов — в заметках), у ИП адрес из документов (реестр даёт только город), КПП у ИП снимается.
- Реквизиты из наших КП/счетов (`clients:extract-requisites`, ежечасно): через ЕГРЮЛ; PDF + **xls/xlsx/docx**; адрес после последнего ИНН/КПП; покупатель привязывается к **реальным получателям письма (To/Cc)**, не к e-mail заявки; наши домены и поставщики из справочника не привязываются.
- **`OrganizationLinkGuard`** (правило заказчика): известный контрагент (есть свои адреса) на чужом адресе — связь не создаётся, менеджеру-отправителю письмо + колокольчик, подтверждение `/dashboard/clients/link/{id}`; без вопросов — новый контрагент, без адресов, тот же корпоративный домен (`CLIENT_LINKS_TRUST_SAME_DOMAIN`). Черновики КП и веб-форма — сомнительное не привязывают молча. Таблица `organization_link_requests`.
- `clients:merge-organizations from:into` (`OrganizationMergeService`); слито 21 двойник; мусорные названия заменены официальными; 3 «переименованных» (МЕИТ СЕРВИС→ЛЕВЕЛСЕРВИС, Импорт-Лифт Сервис→БАСТИОН ЛИФТ, УК Альтернатива→Ривьера Сервис) — старое имя в заметках, **просили проверить глазами**.
- Открыто: 9 без ИНН («Лифт плюс», «Пик Надёжности», «Стакс-плюс», «ЖСК Тюмень», АО «А СПЭЙС», «ЛифтТехСервис» Бузулук, «КубЛифт», «СИБПРОМСТРОЙ №18», «ЛИФТСПЕЦ»); список подозрительных связей `multi_org_suspects.csv` (9 связей у 7 адресов, у заказчика в Downloads) — ждёт решения, какие отвязать; заявки M-2026-5292 / M-2026-8850 (переписка ЛИФТРЕМОНТа вручную объединена с КОМБОЛИФТом 20.07) — не трогали.

**3. Детекторы почты** (каждый раз `mail:detector-replay`, эталон перезаписан; известное расхождение **101479** оставлено видимым — при `--record` восстанавливать ему старое `cited_document`).
- `clientOwnText`: клиент переслал НАШЕ письмо с преамбулой → намерение только из преамбулы (M-2026-17327). Цитата Яндекса без «пишет» (`Кому:/Тема:/время, дата, "Имя" <адрес>:` + `----------------`) срезается (не внутри пересылки). `REPLY_DELIVERY_RE`: «доплат… доставк…», «успеете … отдать/отгрузить» (M-2026-17264). Обе ложные дочерние закрыты `post_sale_correspondence`, письма возвращены в родителей.
- Открыто: M-2026-16066 и M-2026-16693 — ложные дочерние «на счёт» из-за **темы треда** («Просим выставить счёт по КП…», «Коммерческое предложение …»), срабатывание по теме в ответах — следующий кандидат на правку.
- Карточка заявки и PDF переписки показывают «кому/копия».

### Сессии 2026-09-17…24 — кратко (подробности были в локальной памяти прежнего компьютера)
- 09-17: вкладка «Цены к обновлению» и автомониторинг цен; отчёт про автовыдачу КП; ответы поставщиков в чужих тредах (RFQ-токен чужой заявки → роутим по заявке владельца токена).
- 09-18: раздел «Маркетинг» с книжкой контактов; метки и классический просмотр в почте; парные заявки и статус счёта; пул фраз для Директа.
- 09-22: конвейер Яндекс.Директа (mzDirect, YML-фид под токеном, критерий «остаток + актуальная цена»; песочница закрыта до одобрения заявки); блок статистики; флажок и фильтр непрочитанных в почте.
- 09-23: авто-КП в один клик и частичные КП; «Это заявка!» в почте; медиапрофиль с матрицей и памяткой; обратная связь клиентов; раздел «Конкуренты» (отзывы Яндекс.Карт через maps-reviews-widget — отдаёт последние 5).
- 09-24: медиаплан (VK/Telegram/Дзен; публикация только по явной команде заказчика), распределение info@ пропорционально нагрузке, отчёт по дашборду, скидки/организации клиентов, закрепление заказчика за адресом (`client_contacts.pinned_organization_id`).
- Правила авто-КП (заказчик): цена клиента обязательна, неопознанному — розница, у адреса с несколькими юрлицами — лучшие условия, новым отвечаем сразу, OEM не трогаем. «Счёт отправлен»/«КП отправлено» — только по распознанному документу.

### Открыто (2026-09-09)
- ⏳ **Архитектурный разбор конвейера письма** (закрыты 2026-09-09: P0, единые владельцы понятий, таблица решений `mail_decisions`, корпус `mail:detector-replay`, цепочка `RoutingPipeline`; остаются: второй путь создания заявки в персистере (замер по логу), newbie_boost, unique-индекс после слияния дублей 14649/14650 и 2251/2253, 9 постпродажных детей в «ждёт счёт» — закрыть руками) — артефакт https://claude.ai/code/artifact/ca94e2ee-ebb2-42ec-8bec-7f0d5d718c4f. Найдено 13 багов; P0 закрыты `da38324` (см. Закрыто 2026-09-09), остаются P1/P2. Было P0: `services.parser.*` vs `services.mail.parser.*` (пороги парсера ответов и env не работают: `ReplyParseGate.php:63`, `RequestItemPersister.php:109`); `ReassignService.php:54` откатывает статус до assigned при любом переподчинении (в т.ч. sticky по ответу клиента); нет unique-индекса `requests.email_message_id` при трёх входах в `processIfRequest`; `DeliverToManagerInboxJob`/`RouteMailToManagerJob` без `ShouldBeUnique` и без `Bus::chain`, `inbox_deliveries` в JSON без блокировки. P1: blocklist-ветка `MailRouter.php:260` пишет категорию без confidence и блокирует повторную категоризацию; `PostSaleFulfillmentDetector::detect` работает по raw-тексту с цитатой; `ParseRequestItemsJob.php:596/689` ставят assigned через `update()` без аудита/attention; `uniqueId` парсера без force; `MailDeliverToManagerService.php:104` ищет ящик без type/is_active; `MessagePersister::parseRfc2231Filename` static + `$this`. Структурно: `route()` 830 строк/19 выходов/аудит на 4; понятия «свой текст», «просьба о счёте», «дедуп копий», «назначить+доставить» реализованы по 2–4 раза; 24 точки авто-смены статуса; 10/29 кронов — ремонт; 0 тестов на ядро. План: P0 → единые владельцы понятий → цепочка обработчиков + таблица решений + корпус писем.

### Открыто (2026-09-08)
- ⏳ **Backfill Агрызкова остановлен на 29k/99k** — нужные 4458 писем переселены (все, что есть в БД), дальше сплошная история до подключения ящика; при необходимости дозапустить `mail:folders-backfill --mailbox=13`.
- ⏳ **Папки общих ящиков (info@, rfq@) — только в mzCorp**; серверный синк папок сделан для личных ящиков с владельцем (см. «Закрыто 2026-09-08»). Синк не импортирует письма, которых нет в БД (история до подключения ящика в папках Яндекса не появится в mzCorp).
- ⏳ **Флаг ⚑ в личном ящике** теперь показывается как флаг владельца (одна правда на ящик, как прочитанность); директор, ставя ⚑ в чужом ящике, своего результата не увидит. Если понадобится — вынести флаг в отдельный join по текущему пользователю.
- ⏳ **Pull \Seen только для INBOX и синхронизируемых папок за 14 дней** (`ImapSeenSyncService::PULL_DAYS`) и только по личным ящикам с владельцем; общие ящики (info@ и т.п.) двусторонне не синкаются — у них нет «владельца», прочитанность в них per-user.
- ⏳ **Первый pull помечает прочитанными всё, что прочитано на сервере** (у Якубовича — 373 письма разом). Ожидаемо, но бейджи «непрочитанных» у личных ящиков заметно просядут — предупредить менеджеров.
- ⏳ **Drag&drop проверен только программно** (moveToFolder через bulk-меню); реальное перетаскивание мышью в Claude in Chrome не эмулируется — попросить менеджера проверить в браузере.

### Открыто (2026-09-07)
- ⏳ **Композер в карточке заявки** (`Requests\Mail\ComposeForm`) — по-прежнему plain-text `bodyText`; перевести на `mailEditor` (TipTap), как в почтовом клиенте. Также в почтовом клиенте нет BCC (нет колонки), автодополнения адресатов и вставки шаблонов ответов.
- ⏳ **`peak_status` не откатывается ручной сменой статуса** — ложная веха детектора остаётся на чипе после ручного возврата (M-2026-14608 чинили руками). Обсудить пересчёт peak при ручном переходе назад.
- ⏳ **Auto-закрытые заявки и ответы клиента** — `InboundReplyLinker` Guard A блокирует привязку ответа к авто-закрытой заявке (system_close_lost без source message), поэтому ответ клиента на наше напоминание идёт через `CitedOutboundQuoteRouter` или создаёт новую заявку. Стоит пересмотреть: ответ в тред нашего напоминания — надёжный сигнал для реанимации.
- ⏳ **Колонка «писем» на /dashboard/usage** — считает письма клиентам и поставщикам вместе; у Курзаева 177 из них 135 RFQ. Разделить на «клиентам / поставщикам».
- ⏳ **Показатель «В CRM» в отчёте руководителя завышен** — включал авто-уведомления (35–45% исходящих). Реальная дисциплина по собственным ответам в августе 35%, не 63%; у Курзаева 1%, не 41%.
- ⏳ **Pre-existing красные unit-тесты**: `InboundIntentClassifierEligibilityTest` (3 кейса, провайдер не обновлён после расширения ELIGIBLE — есть чип задачи) и `MarketingBlockServiceTest` (2 error — ходят в прод-БД, локально VPN не пускает).

### Открыто (2026-09-04)
- ⏳ **IQOT считает наше предложение «MyLift» конкурентом** — в 10 отчётах среди офферов есть мы сами; `IqotPosition::priceComparison()` не исключает их → ранг сдвинут, бейдж «2-е место, Δ0%» вместо «1-е». Фикс — фильтр по имени поставщика (regex `mylift|мой\s*лифт|мой\s*зип|myzip`) в `offers()`/`priceComparison()` + пересчёт кеша `cmp_*`.
...
- ✅ **Пересылка клиентом своего запроса другому поставщику уходила в постпродажу** (письмо 103750, Швецов → info@, «есть в наличии … цена и сроки поставки» по кнопкам KONE; стадия `post_sale_unlinked`, заявки нет). Причина: `PostSaleFulfillmentDetector::deliveryStatusInquiry` включает ветку `REPLY_DELIVERY_RE` («срок поставки», «когда отгрузите») при `isReply=true`, а `EmailTextCleanerService::isReply` — это просто «есть In-Reply-To ИЛИ тема начинается с Re/Fwd». Яндекс при пересылке ставит In-Reply-To на СОБСТВЕННОЕ письмо клиента → первичный запрос цен читался как вопрос об отгрузке. Фикс: ветка требует `repliesToOurThread()` — заявка уже привязана либо In-Reply-To есть у нас в базе (protected, fail-soft при недоступной БД). Replay по 60 дням до деплоя: вердикт меняют 11 писем из 1455, все без заявки и все запросы цен (97825 датчик Schmersal, 92167 подшипники, 98856 КП для ЛГОКа, 85946 башмак двери и т.д.). Корпус 542/542 без расхождений. Письмо 103750 перепрогнано → заявка **M-2026-15460** (Курзаев, 2 позиции, обе сматчены с каталогом). Перепрогнаны письма не старше недели (решение заказчика 2026-09-10): 97825 → **M-2026-15462** (Румянцев, датчик Schmersal BN 80-RZ), 101803 → **M-2026-15464** (Курзаев, кнопка KONE KM51253859Р04), 98856 «Запрос КП для ЛГОКа» → конвейер сам определил принадлежность к закрытой успехом `M-2026-9323` (Агрызков), привязал письмо как постпродажу, временную заявку удалил без следов в аудите — клиент ждёт сроки по тросу, ответить надо руками. **Хвост:** 7 писем прогона старше недели (27.07–31.08) не трогали.
- ⚠️ **Веерные запросы закупщиков, где мы в скрытой копии, теряются.** Письмо 102912 (Траст-Лифт, кнопка ThyssenKrupp) было адресовано `shop@yamagi.ru` — это ЧУЖОЙ магазин; в открытых получателях нас нет, поэтому `UnintendedRecipientDetector` глушит письмо. Замер: за 90 дней 43 таких письма на чужие адреса (revator.ru, snab-lift.ru, liftformat.ru, propribory.ru, yamagi.ru), из них 41 — ПЕРВОЕ письмо в треде без Re/Fwd, то есть обычные запросы цен от лифтовых компаний, разосланные нескольким поставщикам сразу. Кейс-прародитель гарда `M-2026-1491` (письмо 2943) — наоборот, `Re: Request (…)` с непустым `in_reply_to` в чужом треде. Предложение (ждёт решения РОПа): добавить в `looksLikeBccBlast` третий паттерн «первое письмо в треде + нет Re/Fwd в теме + в to только неизвестные нам адреса» → отдавать классификатору. Заявка `M-2026-15402` создана из такого письма до отката yamagi.ru — судьба зависит от того же решения.
- ✅ **Письма на наши домены mylift.ru / yamagi.ru глушились как «не нам»** (кейс: письмо 102912 «Кнопка 1 этажа лифт ThyssenKrupp» от pn@trastlift.ru на shop@yamagi.ru → чип «Категория письма не предполагает заявку»). `UnintendedRecipientDetector` считает письмо чужим, если ни один адрес из to/cc не наш; «наши» = `services.mail.internal_domains` + Mailbox.email + User.email, а в конфиге стоял ОДИН домен `myzip.ru`. Почта на info@mylift.ru и shop@yamagi.ru форвардится в info@myzip.ru, поэтому такие письма детерминированно уходили в `irrelevant` (confidence 1) ДО LLM, стадия маршрутизатора `not_a_request`. Замер: за 30 дней 115 писем с этим обоснованием, из них 93 адресованы нашим доменам (mylift.ru 91, yamagi.ru 2). Фикс: на проде в `.env` добавлено `MAIL_INTERNAL_DOMAINS=myzip.ru,mylift.ru,mzcorp.ru` (**yamagi.ru НЕ наш** — заказчик поправил 2026-09-10, домен убран; mylift.ru наш) (бэкап `.env.bak-2026-09-10`), в `.env.example` строка раскомментирована с пояснением; конфиг на проде НЕ кешируется, потребовался только `queue:restart`. Перепрогон `mail:requalify-irrelevant --days=1`: 3 письма, из них 102912 → заявка **M-2026-15402** (Васюхно, «Кнопка 1 этажа лифта ThyssenKrupp»), два платёжных уведомления Liftway → post_sale, одно осталось irrelevant. **Хвост:** за 30 дней в заглушённых остаются реальные запросы на mylift.ru — 79855 «ролик с натяжителем PAPPAS», 82012 «Запрос O12043», 80871 «FW: Запрос КП», 73450 «Запрос на КП», 93539 «Ключи для запуска эскалатора Sjec», 85957/86001 «ТРК Акватория canny elevator»; остальное рассылки. Перепрогонять старьё — решение РОПа (`mail:requalify-irrelevant --days=N`).

### Сессия 2026-09-15/16 — итог

Два изменения кода на проде, разбор трафика и три товарных отчёта.

**Код.** `76eec8d` — поиск запросов поставщикам по M-артикулу (`App\Livewire\Suppliers\Index`): раньше искало только по e-mail, названию и теме, теперь заходит в позиции запроса — SKU, артикул из письма клиента, код заявки — и понимает кириллическую «М». Позиции в запросах фиксируются только с июля 2026: из 5010 запросов у 2315 они есть, у 2219 нет ни позиций, ни заявки — такие по M-номеру не найдутся, для 476 с заявкой сделан помеченный фолбэк через неё. `b8f39c8` — utm-метка на ссылке рекламного блока (`MarketingBlockService::withUtm`, `mylift_crm / email / promo_block / block-{id}`); ставится в `renderBlock()`, поэтому превью в админке и тестовая отправка статистику не пачкают.

**Трафик.** На сайте стоит Calltouch, и его данные дважды опровергли мои же выводы: сначала утверждение «атрибуции нет физически», потом «реклама конвертирует в десять раз лучше поиска». Журнал лидов показал, что **динамический коллтрекинг включён только на платный трафик** (26 из 27 звонков рекламные), все обращения с настоящих форм идут без источника, а из 1001 письма на `info@` ни одно не связано с сессией. Сравнивать каналы по этим данным нельзя. Ручное сведение воронки: 134 рекламных лида за 3,5 месяца → 27 новых клиентов → 346 423 ₽ оплачено при расходе около 780 тыс.

**Товарные отчёты.** Цепи эскалаторов (4 типа), тяговая цепь ступеней отдельно, и общий обзор спроса: 13 251 заявка за 01.06–16.09, win rate 23%, по маркам разброс 22–33%, по категориям трёхкратный — поручни 35%, канаты 12%. Для обзора 201 тип каталога свёрнут в 17 категорий.

**Два правила, которые стоит помнить при любой товарной аналитике** (подробно в личной памяти, `catalog-demand-analysis-rules`): поле `catalog_items.brand` содержит вперемешку производителей оборудования, производителей комплектующих и поставщиков — марку машины надёжнее брать из названия позиции; счета-документы дублируются при пересылке (291 документ на 211 счетов), поэтому суммы по строкам считать только по последнему счёту заявки.

Личная память: `session-2026-09-16-summary.md`.

### Сессия 2026-09-11 — итог (аналитика, код не менялся)

Целиком аналитический день: прод-данные читались скриптами через `tinker`, результаты отдавались артефактами и CSV, изменений в коде и БД не было.

**Отчёт по поручням эскалаторов** (01.06–11.09): 344 заявки, предложение по 293 (85%), счетов 151 на 19 240 093 ₽, оплачено 90 на 11 481 544 ₽; строки именно поручней — 9 456 947 ₽ из оплаченных. Недельный график с накоплением, охряная ординальная шкала прошла валидатор палитры. Артефакт `d6481bc6`, список из 51 заявки без предложения — `03dd1e76`.

**Исследование «Цены, которые стоит обновить»** (`f2266eb4`): когорта из 330 заявок, где цена была неактуальна, против контроля в 10 231 заявку — **win rate 26% против 27%**, но до КП доходит 30% против 72%, среднее время первого КП 33,8 ч против 22,7. Исход цикла: цены обновили — 28%, ждём поставщика — 14%, отказ — 0%. Сценарий недополученных продаж **5,0–9,5 млн ₽ за 3,5 месяца** (18,5% спроса доходит до счёта, 57% счетов оплачивается). Пулы: 1 885 позиций на обновление цен, 572 без оборота, **173 позиции на складе, которые спрашивали и ни разу не предложили** (`f929340e`), каждый список разложен на два файла — подтверждено M-артикулом и только OEM.

**Методика уточнялась пять раз по проверкам заказчика** — он открывал конкретные позиции (M15555, M22311, M20503, M03260) и показывал, что фильтр цепляет лишнее. Итоговые правила вынесены в личную память (`catalog-demand-analysis-rules`): товарный срез по `catalog_items.part_type`, а не по слову в названии; спрос засчитывать только там, где артикул позиции реально есть в тексте строки; счёт = выданное предложение; снятые с производства и заменённые позиции из пулов исключать; каталожная цена — прайсовая, а не продажная (расхождение с документами −15…−17%).

**Архитектурный пробел, который вскрылся:** истории флага `catalog_items.is_price_actual` нет — импорт переписывает его на месте. Из-за этого «цена была неактуальна на момент заявки» восстанавливается только косвенно, по `requests.price_refresh_state`. Если такие разборы будут повторяться, стоит начать писать изменения флага в историю, как уже делается для цен в `catalog_price_changes`.

Личная память: `session-2026-09-11-summary.md`.

### Сессия 2026-09-10 — итог

Два разбора писем, оба вскрыли системные дыры приёма почты, обе закрыты в тот же вечер.

**Первая:** список наших доменов состоял из одного `myzip.ru`, поэтому `UnintendedRecipientDetector` глушил всё, что адресовано на `info@mylift.ru` — 91 письмо за 30 дней. Добавлены `mylift.ru` и `mzcorp.ru`. Ошибка по ходу: сначала записал туда же `yamagi.ru` (решил, что наш, раз почта падает к нам) — **заказчик поправил: yamagi.ru чужой**, домен убран из `.env` и `.env.example`. Урок: принадлежность домена спрашивать, а не выводить из того, что письмо дошло — форвард стоит на 16 тыс. входящих за месяц.

**Вторая:** пересылка клиентом собственного запроса другому поставщику читалась как ответ в нашем треде (Яндекс ставит In-Reply-To на его же письмо), и фраза «цена и сроки поставки» уводила запрос в постпродажу. Ветка теперь требует `repliesToOurThread`.

**Открыто и ждёт решения РОПа:** (1) веерные запросы закупщиков, где в открытых получателях чужой магазин, а мы в скрытой копии — 43 письма за 90 дней, 41 из них первое в треде; предложен третий паттерн в `looksLikeBccBlast`; (2) судьба `M-2026-15402` (кнопка ThyssenKrupp от Траст-Лифта), созданной пока yamagi.ru ошибочно числился нашим; (3) перепрогон заглушённых писем на `mylift.ru` за месяц — там 7 реальных запросов (ролик PAPPAS, O12043, «FW: Запрос КП», ключи Sjec, ТРК Акватория, «Запрос на КП»); (4) 7 писем постпродажного прогона старше недели.

Личная память: `session-2026-09-10-summary.md`.

### Сессия 2026-09-09 — итог

План разбора конвейера письма выполнен целиком в порядке 2-3-1-4: журнал решений `mail_decisions`, корпус `mail:detector-replay` (542 письма, эталон на проде), цепочка `RoutingPipeline` из 15 обработчиков вместо `route()` на 830 строк, панель «Качество почты» `/dashboard/mail-quality`. Сверх плана: рекламный блок теперь решается контекстом письма, а не карточкой адресата (+94 письма в сутки); потолок сложности заявок на менеджера (`ManagerComplexityGate`, по умолчанию выключен); три разбора заявок — `M-2026-15205` (дочерняя «на счёт» вместо новой заявки), `M-2026-14316` (счёт на второй адрес клиента; после починки счёт создан и 1С подтянула оплату, заявка закрыта успехом), `M-2026-11924` и ещё пять заявок с ложным «ждёт счёт» возвращены в «КП отправлено». Все три — следствия старых версий правил, сегодняшняя логика их не воспроизводит.

Проверено и работает без правок: оценка сложности заявки (веса по «как пришла позиция», снимок входной сложности) и чёрный список артикулов каталога `catalog_sku_blacklist` (686 SKU, 0 дрейфа, фильтр срабатывает на каждом импорте). У чёрного списка нет интерфейса, обратной операции и автора записи — если пользоваться регулярно, это следующий шаг.

Личная память: `session-2026-09-09-summary.md`.

### Закрыто 2026-09-09
- ✅ **Ложный «ждёт счёт» по цитате КП — 6 заявок возвращены в «КП отправлено»** (`M-2026-3080`, `11924`, `3305`, `12803`, `5862`, `14207`, события `invoice_request_false_positive` от 2026-09-09). Разбор `M-2026-11924` (Румянцев): 20.08 клиент ответил на авто-напоминание ДВУМЯ ФОТО без единого слова своего текста, тема «Re: 364050» → старое правило «цитата КП + вложение = просит счёт» воскресило авто-закрытую заявку и поставило «ждёт счёт»; LLM в ту же минуту дала `inbound_unclear` 0.9, но правило было сильнее. Сегодняшняя проверка намерения даёт `invoice_intent=false` — новых таких не будет. Скрипт отбора: последнее событие `invoice_requested_cited_quote`/`reanimate_from_cited_quote` + перепрогон `CitedOutboundQuoteRouter::detect` по исходному письму; из 9 заявок сегодняшняя логика подтвердила 3, шесть откачены через `RequestStateService::transitionTo(Quoted, systemTransition: true)` с комментарием для менеджера. Внимание пересчиталось в `awaiting_client`, peak_status сохранён.
- ✅ **Счёт на второй адрес клиента не распознавался** (`M-2026-14316`, письмо 99635 «Счет МЗ-9375» ушло на vershina2004@yandex.ru, а в заявке client_email = rodionshvedchikov@yandex.ru). Гард `InternalSenderDetector::isAddressedToClient` (введён для 14608 — пересыл третьей стороне) принимал только точный адрес клиента или тот же КОРПОРАТИВНЫЙ домен; оба адреса на yandex.ru → детектор исходящих не запускался: ни ai_decision, ни парсера вложения, статус завис на «ждёт счёт», счёт не попал в раздел «Счета». Добавлена третья ветка: получатель, чьё ВХОДЯЩЕЕ письмо привязано к этой же заявке, считается клиентом (второй контакт: снабжение, бухгалтерия); третья сторона из 14608 не проходит — она в заявку никогда не писала. Замер за 60 дней: гард отсекал 1284 исходящих, из них с документом и без решения 5, ветка «писал в тред» покрывает 2 реальных, 3 остальных (поставщик eastelevator.cn и т.п.) остаются отсечёнными. Корпус `mail:detector-replay` 542/542 без расхождений. Данные: `quotes:detect-missed-outbound --request=M-2026-14316 --apply` создал счёт №9375 (6949,68), импорт 1С сразу подтянул оплату от 08.09 (журнал #4647) → заявка авто-закрыта успехом; ai_decision осталась dismissed («запрещённый переход closed_won → invoiced») — ожидаемо. Хвост: `M-2026-11540` письмо 99845 со «Счет МЗ-9241» тоже не распознано, у заявки уже зарегистрирован счёт 8769 — прогонять руками по решению РОПа.
- ✅ **Потолок сложности заявок на менеджера** (`users.max_complexity_level`, `users.only_internal_sku_requests`; `ManagerComplexityGate` + карточка менеджера + чип в списке). Запрос РОПа: отстающему давать только простые заявки, в идеале только с M-артикулами, и поднимать планку по мере роста. Правило применяется ТОЛЬКО в round-robin: sticky (личный ящик, свой клиент, тот же товар) сильнее, при пустом списке кандидатов фильтр снимается, заявка без позиций ограниченному не отдаётся. Кого отсёк потолок — в `request_assignments.reason` (`gate`) и в логе `AssignmentService: complexity gate applied`. По умолчанию ограничений нет, распределение не меняется. Замер на проде: свободного потока за 30 дней 2218 заявок, при потолке «лёгкие» доступно 73%, «средние» 95%, «только M-артикулы» 31%; sticky ещё 1454 заявки мимо фильтра. Сложность известна к моменту назначения в 95% случаев (1598 из 1685 за 14 дней).
- ✅ **Рекламный блок: решает контекст письма, а не карточка адресата** — `MarketingBlockService::isEligible` больше не выключает блок только потому, что адресат есть в справочнике поставщиков. Причина: контрагент бывает и клиентом, и поставщиком (Liftway, Meteor, nlp-group; 37 заявок за 30 дней с client_email из справочника). Новое правило: RFQ (`supplier_inquiry_id`) — без блока; все получатели внутренние — без блока; письмо по клиентской заявке её клиенту (`related_request_id` + совпадение с `client_email`) — с блоком, даже если адрес числится поставщиком; по заявке на сторонний адрес: поставщику — без, второму контакту клиента — с; без заявки — прежнее правило по справочнику. Замер на проде за сутки: было с блоком 210, станет 304 (+94, из них liftway.store 60, meteor.ru 20), без блока остаются 54 запроса поставщикам. Тесты `MarketingBlockServiceTest` переведены на стаб `SupplierRegistry` и отношение `relatedRequest` — 14/14 зелёные локально (прежние 2 «красных по БД» больше не красные).
- ✅ **closed_won: дочерняя «на счёт» только по просьбе о счёте** (`M-2026-15205`, письмо 101843 «Дайте КП на отводки… 1 шт» с номером старого КП в теме → родилась дочерняя на счёт по старому КП). Правило в `ClosedWonThreadHandler`: `wantsNew` (не вопрос о сроках) и внутри него `asksInvoice` (InvoiceMentionMatcher по subject+свой текст) → дочерняя с позициями из КП; `wantsNew` без просьбы о счёте (новое КП / другие позиции) при category=client_request или цитате КП → spin-off отдельной заявки. Replay по 35 письмам-источникам дочерних: 7 дочерних, 4 spin-off, 24 постпродажа. Данные: письмо развёрнуто в M-2026-15237 (Головнев, 3 позиции), 15205 закрыта как дубль.
- ✅ **Панель «Качество почты» `/dashboard/mail-quality`** (`e123fbe`) — `MailDecisionQualityService` + Livewire `MailQuality\Index` (РОП/директор/админ, ссылка в топбаре после «Аналитики»): AI-решения по детекторам с долей отклонённых (dismissed+overridden / рассмотренных), ручные откаты статуса (event=manual из quoted/under_review/awaiting_invoice/invoiced → assigned/in_progress/awaiting_clarification/under_review), заявки-фантомы (closed_lost с причиной duplicate/post_sale_correspondence/off_topic/parser_no_content/supplier_reply), стадии `mail_decisions`, понедельная динамика. Ориентиры в шапке: отклонённых <10%, откатов <100/мес, фантомов <3%. Отчёт за 30 дней — 0.12 с. **Уточнение цифры из разбора:** «312 откатов за 30 дней» считались по любым переходам назад, включая системные (llm_reassess_revert, ai_auto_apply); ручных (event=manual) — 17 за 30 дней у 6 менеджеров, фантомов 155 из 3680 (4,2%), самый шумный детектор — `inbound_invoice_request`.
- ✅ **`MailRouter::route()` → цепочка обработчиков, перенос завершён** (`bceb83b`, `36da9f8`, `0e8561e`) — `RoutingPipeline::HANDLERS` (15, порядок — контракт): SystemNotification → RfqInbox → Outbound → NotInbound → LoopForward → ProcurementMailbox → Blocklist → CrossMailboxCopy → SupplierReply → Categorize → LinkToRequest (линкер + цитата КП, `CitedOutboundQuoteRouter::applyInvoiceRequest`) → ClosedWonThread → PostSaleOrder → LinkedThread → CreateRequest (всегда решает). `RoutingContext` несёт `linkedRequest`/`postSaleUnlinked`; `RoutingDecision(stage, requestId, payload, runRules)` — правила forward/label запускаются только для «хвостовых» решений (thread_reply, request_*, not_a_request, post_sale_unlinked), как раньше. `MailRouter` — 151 строка (pipeline → `MailDecisionRecorder` → правила), `runOutboundDocumentDetection` делегирует в `OutboundDocumentDetectionService`. Постпродажная доставка теперь тоже цепочкой Deliver→Route. Каждый шаг деплоился отдельно: tinker-резолв всех handler'ов ДО `queue:restart`, 5 минут прода по `mail_decisions` (все стадии пишутся, 0 «MailRouter failed», routed_mails растут только на хвостовых решениях). **Новая ветка маршрутизации = новый handler в HANDLERS, не ветка в route().**
- ✅ **Цепочка обработчиков вместо `route()` — шаг 1 (strangler)** (`da9dd44`) — `App\Services\Mail\Routing\{RoutingContext, RoutingDecision, InboundRoutingHandler, RoutingPipeline}` + 8 обработчиков в `Routing\Handlers` (SystemNotification, RfqInbox, Outbound, NotInbound, LoopForward, ProcurementMailbox, Blocklist, CrossMailboxCopy), код перенесён дословно; детектор исходящих документов вынесен в `OutboundDocumentDetectionService` (`MailRouter::runOutboundDocumentDetection` делегирует — публичная точка для `OutboundReplyHooks` и `quotes:detect-missed-outbound`). `route()` = `pipeline->run()` + остаток (матчинг поставщиков → категоризация → линкер → closed_won/post_sale → ветка привязанной заявки → создание) как раньше. Перенос делался скриптом с якорями (`scratchpad/strangle1b.php`). Проверка: контейнер собирает роутер и все обработчики (tinker до `queue:restart`), 5 минут прода — 0 «MailRouter failed», решения пишутся по всем стадиям. Порядок `RoutingPipeline::HANDLERS` — контракт. Дальше: шаг 2 — три матчера поставщиков, шаг 3 — категоризация+линкер+closed_won/post_sale, шаг 4 — ветка привязанной заявки и создание; после каждого — тот же контроль по mail_decisions.
- ✅ **Корпус реальных писем как регрессионный тест — `mail:detector-replay`** (`f151bdf`) — read-only реплей детерминированных детекторов (is_reply, post_sale detect, deliveryStatusInquiry, requestsInvoiceToPay, InvoiceMentionMatcher::mentions, wantsNewInvoiceOrOrder, CitedOutboundQuoteRouter::detect + invoice_intent) по корпусу писем с эталоном `storage/app/mail-corpus/baseline.json` на проде (не в git). Корпус: 166 писем с цитатой КП + 98 постпродаж по closed_won + 34 источника дочерних заявок + кейсы 101376/97620 + 300 случайных входящих за 30 дней = 542; `--record` пишет эталон, без флагов сравнивает и возвращает exit 1 при расхождениях; 21 с на прогон. **Правило:** перед деплоем любой правки детекторов/очистки текста — `php artisan mail:detector-replay` на проде, расхождения разобрать глазами (как сегодня), затем `--record` нового эталона. После больших изменений корпус дополнять свежими кейсами (`--ids=... --sample=N --record`).
- ✅ **Журнал решений маршрутизатора `mail_decisions`** (`b405eaf`) — `MailDecisionRecorder::record()` (fail-soft) вызывается из `MailRouter::decide()` перед каждым из 18 early return'ов `route()` (стадии: system_notification, rfq_inbox, outbound, not_inbound, loop_forward, procurement_mailbox, blocklist_supplier/spam, cross_mailbox_copy, supplier_thread/sender_code/rfq_subject/rfq_unregistered, closed_won_spin_off/invoice_child/post_sale, post_sale_order, thread_spin_off) и `recordFinalDecision()` в конце (thread_reply / post_sale_unlinked / request_created / request_existing / request_rejected / not_a_request). Поля: stage, outcome (skipped/supplier/copy/post_sale/linked/created/outbound/none), request_id, category, reason (подпись из `MailDecisionRecorder::STAGES`), payload. В почтовом клиенте под «Привязано к заявке» — чипы последних 3 решений. Вставка делалась скриптом по номерам строк с проверкой, что строка — `return;` (`scratchpad/insert_decisions.php`). Дальше: пункт 3 плана — корпус писем как регрессионный тест (теперь у него есть «ожидаемое решение» = stage).
- ✅ **M-2026-8429: новая заявка в треде выигранной сделки принята за постпродажу** — письмо 101376 (10:34, ДО сегодняшних деплоев): LLM client_request 0.95 «новый запрос», линкер L1 прикрепил к closed_won, ветка closed_won не нашла номера КП и принудительно поставила post_sale (так с 19.08, `cacc5d9`; вчерашний гейт работает только при найденном номере КП). Фикс: нет цитаты КП + категория client_request + `wantsNewInvoiceOrOrder` по своему тексту + `ReplyParseGate::shouldParse` → `RequestExtensionService::spinOffNewRequest` (как intent=new_request). Реплей за 30 дней: 98 постпродаж по closed_won, 18 с wants=true, из них настоящий новый заказ — один (этот); `shouldParse` сам по себе слишком мягкий (true у 14 из 18) — решает связка с вердиктом LLM. Данные: письмо развёрнуто в M-2026-15201 (6 позиций, sticky direct_mailbox → Головнев).
- ✅ **Единые владельцы «положить письмо в папку менеджера» и «это ответ?»** (`ae87ee2`) — `ManagerMailRoutingService::routeOrRetry()/isAlreadyRouted()` вместо четырёх копий (персистер + три места в `ParseRequestItemsJob`), поведение прежнее (sync UID MOVE → на TransientImapException async retry +30 с, прочее — warning); `EmailTextCleanerService::isReply()` вместо трёх одинаковых regex (`InboundReplyLinker::subjectLooksLikeReply` намеренно строже — оставлен). Второй путь создания заявки в `RequestItemPersister` (status New, без организации) пока не удалён — добавлен `Log::warning('RequestItemPersister: request created by the secondary path')` для замера; за 30 дней `system_initial` с to_status=new — 0, 22 записи «null→new» — это split-заявки. Если за неделю warning не появится — путь удалить в пользу `IncomingMailProcessor`.
- ✅ **Единый владелец «просьба о счёте» — `InvoiceMentionMatcher`** (`3742a82`) — `requestsInvoice()` (глагол-просьба с запятыми/«пожалуйста», «прошу/можно/направить/выписать/поправить … счёт», «счёт на <товар/оплату/N>», «счёт по КП», минус «счёт-фактура» и реквизиты выставленных документов), `intendsToPay()` («будем оплачивать», «завтра оплата Вам»), `mentions()`, `referencesIssuedInvoice()`. На него переведены `PostSaleFulfillmentDetector::looksLikeInvoiceRequest`, `CitedOutboundQuoteRouter::hasInvoiceIntent` (раньше любое «счёт/оплат/выстав» → «ждёт счёт») и `InboundIntentClassifier::mentionsInvoiceToken`. Реплей на 166 письмах с цитатой КП за 30 дней: v1 (строгая) теряла половину настоящих просьб (запятые, «пожалуйста», «прошу счёт», «Счет на Табло … КП №»), v3 — 30 расхождений, все ложные «ждёт счёт» по постпродажным вопросам, настоящие просьбы сохранены. 18+11 кейсов в `InvoiceMentionMatcherTest`. `PaymentDocumentDetector::extractInvoiceNumber` (номер из платёжки) и `KEYWORD_RE` роутера (контекст для матча номера) — другие понятия, не трогались.
- ✅ **Единый владелец «собственный текст клиента»** (`e41a420`) — `EmailTextCleanerService::clientOwnText(EmailMessage)`: срез любой цитаты (`cutQuotedReplyTail`), блок пересылки при преамбуле, html-fallback при битом plain. На него переведены `CitedOutboundQuoteRouter::ownBodyText`, `PostSaleFulfillmentDetector::freshText` (раньше `cutOwnQuotedTail` — только наша цитата) и `InboundIntentClassifier::mentionsInvoiceToken` (раньше свои маркеры). Реплей на проде, 30 дней / 16 605 входящих: `requestsInvoiceToPay` — 224 эффективных расхождения (при post_sale/override), все просмотренные 14 — старые ложные срабатывания на «счёт» из цитаты (постпродажные вопросы уходили в client_request → заявки-фантомы); `detect()` — 279 расхождений, из 61 новых срабатываний на client_request/thread_reply просмотрено 18: 17 — настоящая постпродажа («счёт в оплате, прошу переместить на отгрузку», «срок поставки по счёту»), 1 ложное (техответ 71824). Метод реплея: скрипт с inline-копией старой и новой логики через `tinker file.php </dev/null`, сравнение только там, где результат реально влияет. 5 unit-тестов `EmailTextCleanerClientOwnTextTest`.
- ✅ **Конфиг после разбора** — добавлены ключи `services.openai.intent_model` (7 мест читали с inline-дефолтом) и `services.mail.outbound_link_subject_similarity_threshold` (0.5), удалён мёртвый `outbound_link_window_days`; значения не изменились. **Намеренно НЕ сделано:** перевод `PostSaleFulfillmentDetector::detect()` на текст без цитаты — реплей по 4 298 входящим за 30 дней дал 5 изменений вердикта (3 лучше, 2 хуже, все 5 и так post_sale по LLM) — эффект нулевой, риск ненулевой. Также не трогались: мёртвые ветки `route()`, настройка `newbie_boost` в админке (P2), unique-индекс `requests.email_message_id` (заблокирован двумя историческими дублями 14649/14650 и 2251/2253 — сначала слить руками).
- ✅ **P0-фиксы по разбору конвейера** (`da38324`) — пороги парсера ответов читаются из `services.mail.parser.*` (значения не изменились: env на проде нет); `ReassignService` больше не откатывает статус (только New→Assigned, Pending остаётся); создание заявки в `IncomingMailProcessor` и `RequestItemPersister` под `lockForUpdate` строки письма с перепроверкой привязки (реальная гонка: M-2026-14649/14650 созданы в одну секунду двумя путями, обе в quoted — не сливались, решать руками); `DeliverToManagerInboxJob`/`RouteMailToManagerJob` — `ShouldBeUniqueUntilProcessing` по (письмо, менеджер) + `Bus::chain` Deliver→Route с catch-fallback (`DeliverToManagerInboxJob::chainWithRouting`) во всех трёх местах диспатча; `inbox_deliveries` пишутся под блокировкой по свежим artifacts; ящик менеджера ищется только среди personal; `ParseRequestItemsJob::uniqueId` учитывает force/reset, оба `update(['status'=>Assigned])` заменены на `RequestStateService::systemAssign` (аудит `system_assign` + attention); `MessagePersister::safeConvertToUtf8` static; blocklist-ветка пишет полный набор полей категории. Проверка на проде 12:25–12:40: 0 failed jobs, цепочка Deliver→Route выполняется по порядку (лог msg 101687), синк всех ящиков идёт.
- ✅ **M-2026-14700 «ждёт счёт» на вопрос о сроках по выигранной сделке** (`207ed73`) — ответ клиента в тред closed_won M-2026-6332 («уточнить дату готовности заказа по счету № 6811») линкер привязал верно, но ветка MailRouter «цитата нашего КП по closed_won → дочерняя заявка на счёт» заводила ребёнка по одному факту номера КП в письме, не глядя, просит ли клиент счёт. За авг–сен так родились 34 ребёнка, ~15 из них — постпродажные вопросы («когда получим», «по срокам успеваем?», «заберём завтра», «аннулировали документы»). Фикс: гейт `PostSaleFulfillmentDetector::wantsNewInvoiceOrOrder(subject, ownText, isReply)` по СОБСТВЕННОМУ тексту клиента (`CitedOutboundQuoteRouter::ownBodyText`): статус/сроки/самовывоз → постпродажа; «выставить/дайте/нужен счёт», «N шт», «продать/купить», «готовы оплачивать», «обновить КП» → ребёнок. Проверено на всех 34: каждый wants=true дошёл до invoiced/closed_won с документами, каждый wants=false — постпродажа. 16 data-provider тестов. Данные: 5 фантомов без активности менеджера (14340, 15154, 14183, 14700, 14864) закрыты `system_close_lost`/`post_sale_correspondence`, письма возвращены родителям с post-sale attention; 9 постпродажных детей с ответами менеджера (13923, 13925, 13973, 14012, 14235, 14236, 14253, 14346, 14654) оставлены в «ждёт счёт» — закрывать руками по усмотрению.

### Закрыто 2026-09-08
- ✅ **Сторож очередей + отдельный default-first воркер** (`4042908`) — по `/var/log/mzcorp/worker.log` воркеры НЕ висели: с 14:32 до 16:12 они непрерывно молотили `mail-sync` (SyncMailboxFolderJob по 16–21 с + мои SyncImapFoldersJob, в т.ч. 3-минутные таймауты по ящику 13), `default` не доставалось ни разу. Сделано: `queue:watchdog` каждые 5 минут (config `queue.watchdog`: default/mail-sync 10 мин, catalog 60; warning в лог + `QueueStalledNotification` админам bell+mail, cooldown 60 мин, вид в bell — kind `queue_stalled`); в supervisor добавлен `mzcorp-default-worker` (numprocs=1, `--queue=default,mail-sync`) — гарантированный прогресс default; конфиг в `deploy/supervisor/mzcorp-worker.conf`, применён `cp` + `reread/update`.
- ✅ **Инцидент: info@ — заявки создаются, но письма не уходят в MZ|<менеджер> и остаются непрочитанными (14:36–16:12)** (`0ba16f9`) — очередь `default` (RouteMailToManagerJob, DeliverToManagerInboxJob, ParseRequestItemsJob…) стояла: 145 задач. Две причины: (1) 3 из 4 воркеров зависли внутри задачи (etime 1ч39, CPU 1 мин, `queue:restart` их не берёт — только `supervisorctl restart mzcorp-worker:*`); (2) единственный живой воркер не выходил из приоритетной `mail-sync`, куда с 14:30 каждые 2 минуты падали новые `SyncImapFoldersJob` по 9 ящикам (у Агрызкова ~10 с на getUid папки в 100k). Фикс: синк папок не чаще раза в 10 минут на ящик (`folder_sync_interval_minutes`, Cache::add-троттл в `MailSyncCommand`), воркеры перезапущены, очередь разобралась за ~5 минут, роутер снова переносит (лог `MailFolderRouter: moved (UID MOVE)`). Причина зависания воркеров не установлена (webklex ставит stream_set_timeout 30 с, pcntl есть) — см. «Открыто».
- ✅ **«Заявку переназначили, письмо из Яндекса исчезло, а у нас осталось» + «сверь последовательность писем с Яндексом» (Агрызков)** (`30cfd2b`, `cf8fc71`) — (1) `MailReassignArchiverService` кладёт копию в `MZ|Reassigned` и пишет это в `folder`, но клиент фильтровал «Входящие» только по направлению → `Client::hideReassignedCopies` (пути из `MailReassignArchiverService::archivePaths()`); в личных ящиках было 4–14 таких копий. (2) Реальный INBOX Агрызкова — 39 писем, у нас во «Входящих» 4510: его почтовик перекладывает всё в «Входящие (локально)» (99 837 писем), 4485 наших записей давно не в INBOX сервера. Сделано: `Client::hideGoneFromServer` — в личных ящиках с серверным синком входящие с `imap_uid = NULL` (нет ни в одной видимой папке сервера) не показываются; «Archive» Яндекса стал обычной синхронизируемой папкой (убран из system roots и special-use); команда `mail:folders-backfill --mailbox=N` — разовый полный проход по папке с матчем по Message-ID (≈0.1 с/письмо, для 100k ~3 ч, запущена в фоне `nohup … > storage/logs/backfill-13.log`). Лёгкий `BODY.PEEK[HEADER.FIELDS (MESSAGE-ID)]` webklex не парсит («Empty response») — только `RFC822.HEADER`. Грабли деплоя: `nohup … &` внутри `ssh '…'` держит сессию — запускать через `setsid`/отдельный ssh.
- ✅ **У Курзаева «другой интерфейс почты»** (`3b647ba`) — он единственный с ролью `head_of_sales` (РОП), а почтовый клиент `/dashboard/mail/inbox` был открыт только manager/admin/director; левая панель вела РОПа на старую org-wide витрину `/dashboard/mail`. Открыт клиент РОПу (роут, `Client::canAccess`, left-rail, ссылка «✉ письма» в карточке) и дан полный обзор ящиков (`MailboxAccessService::hasFullMailboxAccess`). Старая витрина осталась секретарю. В БД есть второй тестовый аккаунт «Илья Курзаев» (user 7, man1@myzip.ru, manager) — не путать с боевым user 10.
- ✅ **Бейдж «Входящие» 4503 при 24 реальных (Агрызков)** — pull \Seen шёл только по окну 14 дней, старые письма, прочитанные в Яндексе, висели непрочитанными; плюс тысячи INBOX-записей, чьи UID давно ушли из INBOX сервера (его почтовик перекладывает всё в «Входящие (локально)»). Фикс: раз в сутки полный проход по всей истории ящика (`ImapSeenSyncService::FULL_PULL_EVERY_HOURS`, кеш-ключ `imap-seen-full-pull:<id>`), и в полном проходе записи старше суток, которых в серверной папке больше нет, помечаются прочитанными для владельца с `imap_uid = null` (ушедшее из «Входящих» не висит непрочитанным). Прогон по всем личным ящикам: Агрызков 4471 → бейдж 26, Ежов 188, Якубович 226 — теперь это реальные непрочитанные на сервере. Грабли: webklex `flags()->validatedData()` бросает «Empty response», если ни одного UID из набора нет — ловить вокруг `validatedData()`.
- ✅ **Синк папок с Яндексом для личных ящиков + прочитанность владельца видна всем + поиск по всем папкам** (`1321bd4`, `3afd5f3`) — заказчик: «прочитанность не синхронизирована» (смотрел как админ и видел СВОЮ прочитанность; pull работал — сервер и состояние владельца сходились по всем ящикам), «синхронизировать структуру папок и расположение письма», «поиск по всем папкам с подсветкой папки и фильтром». Сделано: `ImapFolderSyncService` (pull LIST/UID-списки/Message-ID, push CREATE/RENAME/DELETE/UID MOVE через `PushImapFolderOpJob`, `SyncImapFoldersJob` из `mail:sync`), `mailbox_folders.imap_path/imap_synced_at`, `MessagePersister` переселяет вернувшееся в INBOX письмо вместо повторного ingest, `pullSeen` читает флаги и в папках; `Client::readStateUserByMailbox` — прочитанность/флаг личного ящика = владельца для всех зрителей; поиск: `searchIn` (all/inbox/f:<id>), селект «Искать в», чип папки в строке, шапка «Поиск · все папки». E2E на ящике Якубовича: create → LIST видит `&BCIENQRBBEI- &BEEEOAQ9BDo-` (Тест синк), move → uid 1 в папке, pull без расхождений, delete → письмо обратно в INBOX (COPYUID), папка удалена. Инциденты: тестовое письмо 70230 уехало в Trash (DELETE непустой папки) — возвращено; мусорная папка `&-BCIE…` от `ensureFolder` — удалена; 500 на странице почты ~5 мин из-за `bigint = text` в CASE-join — исправлено литералами.
- ✅ **Из карточки заявки → в почту со всеми письмами заявки** (`4f2548b`, `58477a2`, `cd9a277`) — ссылка «✉ письма» в карточке ведёт на `mail/inbox?request=<id>`; клиент показывает чип фильтра и письма по всем доступным ящикам (`Client::$requestId`, `folderQuery` в режиме request), бейджи папок при этом считаются без фильтра.
- ✅ **Почта как у Яндекса: папки/подпапки, выделение писем, \Seen-синк** (`908d74f`, `9cf7516`) — таблица `mailbox_folders` (до 3 уровней, `MailboxFolderService`), `email_messages.mailbox_folder_id`; Ctrl+A/Shift-клик/чекбоксы выделяют письма, bulk-бар «Прочитано / Непрочитано / В папку», drag&drop строк на папки и «Входящие»; `ImapSeenSyncService` — push `\Seen` при открытии/«непрочитано» и pull с сервера из `mail:sync` (`PullImapSeenFlagsJob`), **только для владельца личного ящика** (`isOwner`): директор/РОП в чужом ящике флаги не трогает. Правило 8 CLAUDE.md дополнено «Исключением 2». Проверено на проде: STORE ±\Seen на ящике Якубовича, pull 373/0 за 0.5 с; в браузере — папка создана → письмо перенесено → видно в папке → папка удалена → письмо вернулось; `mail:sync --mailbox=14` ставит 2 job'а, оба отработали без ошибок.

### Закрыто 2026-09-07
- ✅ **«Счёт отправлен» без счёта** (M-2026-14608, `e78e225` + follow-up) — внутренний пересыл письма клиента с PDF «Реквизиты ООО…» прилинковался по In-Reply-To, LLM-классификатор исходящих прочитал цитату клиента как наш счёт → Invoiced. Три слоя фикса: гард «получатель = заказчик» (`InternalSenderDetector::isAddressedToClient`), срез цитаты/пересыла перед LLM (`EmailTextCleanerService::cutQuotedReplyTail`), и главное — **статусы Invoiced и Quoted по детектору ставятся только после разбора документа парсером** (счёт: создан Invoice с номером+позициями+суммой; КП: OutboundQuote с позициями), см. `DetectorType::requiresDocumentEvidence`. Данные заявки поправлены (outbound_quote 13513 удалён, `peak_status` сброшен). См. журнал сессии 2026-09-07.
- ✅ **«Перестали применяться КП» (M-2026-14815)** (`4832380`) — не баг гейта, а плашка AI-подсказки, которую менеджеры жали за 2–20 с, пока парсер работал. Плашка скрыта на время разбора (`AiDecision::PAYLOAD_AWAITING_PARSE_UNTIL`, 15 мин страховка), карточка поллится. Живая проверка: два КП применились сами за 6–8 с.
- ✅ **Персональные отчёты 7 менеджеров за август** — `Downloads/mzCorp_отчет_<Фамилия>_август2026.md`, генератор и снимок данных в `Downloads/mzCorp_reports_generator`. Сверка с отчётом руководителя: заявки и выручка июня–июля сошлись; «В CRM» там включал авто-уведомления (у Курзаева 41% vs 1% собственных ответов) — в отчётах даны обе строки.
- ✅ **Рекламный блок: свой текст ссылки** (`fb9aca4`) — `marketing_blocks.link_text`, пусто → «Подробнее →».
- ✅ **Почта: полноценный редактор письма** (`1c792d8` … `8b6d525`) — TipTap, картинки в теле (inline cid, GD ≤1600px), таблицы (email-safe стили только на текст менеджера — подпись не трогаем), панель в стиле Simple Editor. Грабли: Alpine-proxy на ProseMirror вешал вкладку; `$wire` в x-teleport бьёт в родителя; `wire:loading` в teleport не работает; регистрация `Alpine.data` должна быть независима от порядка скриптов.
- ✅ **Список писем: номер 1С рядом с M-кодом; маркер ₽ и янтарная подсветка строк заявок «ждёт счёт»** (`00adbcc`, `feab24e`/`aaf0c51`, `12bd63b`). Инцидент: perl-замена съела `\App\Enums` → битый blade на проде ~2 мин, откат + фикс через Edit.
- ✅ **Ложное «ждёт счёт» от роутера цитируемого КП** (M-2026-12166, `5704d4b`, `3750cb4`) — интент счёта только из собственного текста клиента; 4 заявки откачены, M-2026-12166 → under_review.
- ✅ **Сотрудники-«поставщики» через ящики снабжения** (`668223b`) — 16 inquiry удалены, 326 писем отвязаны; внутренний отправитель больше не становится поставщиком.
- ✅ **Прод: 217 файлов под root ломали `git pull`** — `chown -R www-data:www-data /var/www/mzcorp`, root-owned = 0.
...
- ✅ **Заявка-фантом из ответа с реквизитом счёта** (`c4cf07a`, M-2026-14524 закрыта как `post_sale_correspondence`).
- ✅ **Письмо в info@ не видно в «Почте», если копия** (`4ba5bc2`, `364b676`) + индекс по маркеру копии.
- ✅ **Уведомления делегату при отпуске** — проверено, работает как задумано; правок нет.

### Закрыто 2026-09-02
- ✅ **Платёжное поручение без треда больше не плодит заявку-фантом** (`8429f37`, фикс `f8acf9b`). Кейс M-2026-14395: мобильная Почта Mail прислала голый PDF (тема = имя файла, тело 34 симв., без In-Reply-To/References) — классификатор содержимое вложения не видит, линкеру зацепиться не за что, парсер платёжку прочитал и выбросил, 0 позиций → авто-ответ + назначение менеджера. Новый `App\Services\Mail\PaymentDocumentDetector`: `record()` пишет вердикт (`doc_type` + номер счёта из текста) в `email_messages.detected_artifacts.payment_document` прямо из `RequestItemParsingService::classifyHeavyAttachmentRelevance`; `handleEmptyRequest()` в `ParseRequestItemsJob` (ветка empty-items, ДО `assignIfStuckPending`) гасит фантом — счёт нашли → письмо на заявку счёта + `closed_lost/duplicate` + attention 🛒 менеджеру, не нашли → `closed_lost/post_sale_correspondence` (новый `ClosedLostReason`). Валидировано на реальном прогоне: M-2026-14395 закрылась, переписка переехала на M-2026-12878 (счёт 8772). Подробности, включая два бага, вскрывшихся только на реальных данных (порядок блоков в извлечённом PDF; stale-инстанс `$message`), — в журнале сессий.
- ✅ **Письмо-инициатор доходит до назначенного, когда владелец личного ящика в отпуске** (`f75b19d`). Guard `origin_in_another_personal` в `MailDeliverToManagerService` глушил IMAP APPEND; его посылка («владелец A видит письмо у себя») неверна для отпуска. Замер до правки: 63 из 63 заявок из ящиков двух отпускников за 14 дней ушли назначенным без письма. Добавлено исключение для `isUnavailable()`/архивированного владельца, добэкфилено 35 писем, 16 легитимных пропусков (доступные владельцы) сохранены.

### Закрыто 2026-06-25
- ✅ **Разбор КП: две колонки «Кол-во» (Vision брал «заказано» вместо «к отгрузке»)**. Шаблон Liftway/МЗ: в области «Кол-во» два числа — «заказано/ожидается» (большее, справочное) и «к отгрузке/гарантировано» (меньшее, фактическое). Сумма строки = `unit_price × вторая колонка`. Vision иногда брал ПЕРВУЮ → завышал `quantity` и сам вычислял `total = unit_price × завышенное_кол-во`. `autoFixRowArithmetic` это НЕ ловил (up×qty==total внутренне сходится). Кейс M-2026-4755 (quote 3122, поз.13 M24157): up=1434.42, «21,00/1,00», Сумма=1434.42 → модель qty=21,total=30122.82, переоценка ровно up×20=28688.40 = остаточный `sum_vs_total_mismatch` +6.1%. Три части фикса в `OutboundQuoteParsingService`: (1) **промпт** `buildParsingPrompt` — блок «ДВЕ КОЛОНКИ КОЛ-ВО»: бери Кол-во, при котором `unit_price×qty≈печатная Сумма`, `total`=печатная Сумма (не вычислять); (2) **retry-feedback** `buildRetryFeedbackPrompt` — при `diff>0` (Σ выше итога = сигнатура большей колонки) причина 2b; (3) **дет. сеть** `reconcileDualQuantityColumns`+`numberAppearsInText` — сверяет печатную «Сумму» строки с текстовым слоем PDF: если модельный `total` в тексте нет, а `up×k` (k<qty) есть → правит `qty=k,total=up×k`. Консервативно: только Σ>итога >2%, штучные целочисленные qty≥2, **полный откат** если Σ не сошлась ±2%. `numberAppearsInText` учитывает русский формат (пробел/nbsp/narrow-nbsp как разделитель тысяч, запятая/точка), lookbehind'ы отсекают младшую группу (`461,20` внутри `11 461,20`). Флаг `services.quotes.dual_qty_reconcile` (`QUOTE_DUAL_QTY_RECONCILE`, default on). Тест `tests/Unit/Services/Quotes/OutboundQuoteDualQuantityTest.php` (6 кейсов, зелёный на Herd 8.3.29). **Прод-валидация:** `quotes:parse-outbound --apply --reset --quote=3122` → ожидаем поз.13 qty=1/lt=1434.42, Σ==472668.41, warnings пусты.
- ✅ **Ревайвл: отслеживание падения цен по M-артикулам строк КП** (`6133c56`). Оживляющее письмо описывало позиции по `request_items`/`matched_catalog_item_id` → в текст попадало внутреннее имя каталога, которого клиент в КП не видел (кейс M-2026-4755: SKU в каталоге «Плата DOP-116», в КП «Плата вызова ВВЕРХ»). Теперь источник истины — строки самого КП: `NotificationsDispatchClientCommand::resolveLastSentQuote` отдаёт `lines` (Quotation `snapshot_sku`/`snapshot_name` ИЛИ OutboundQuote `raw_article`/`raw_name`), `collectPriceDrops($since,$pct,$quoteLines)` резолвит каждый M-артикул в каталог по sku и следит за ценой по нему, имя берёт из КП. Без привязки к request_items/матчу. На проде.
- ✅ **Разбор исходящих КП: не терять позиции в длинных сводных КП** (`19355d7`+`8c9a0a1`). Кейс M-2026-4755: клиент прислал 6 фото = снимки 4 ранее выданных документов (24 позиции суммарно) с просьбой «объединить в одно КП»; парсер `OutboundQuoteParsingService` извлёк только **5 из 24** (Σ 312350 при итоге 472668, −33.9%). Четыре фикса: (1) **text-лимит** базового промпта 8000→`config services.quotes.parser_text_limit` (24000) — длинный текстовый слой не доходил до модели; (2) **де-якорь** промпта от `request->items` («список заявки = подсказка, документ может быть шире, извлекай ВСЕ строки»); (3) **retry на пропуск строк** в `buildRetryFeedbackPrompt` (Σ ≪ итога, diff<0, ≥`services.quotes.missing_rows_pct`=10) — добрать пропущенные, а не пересчитывать; (4) **сверка артикул↔текст** `verifyArticlesAgainstText` (M-артикул не в тексте PDF = вероятный мисрид, границы по ЦИФРАМ т.к. smalot склеивает токены). Warning-типы в `payload['warnings']`: `likely_missing_rows`(+`needs_review`+ERROR-лог), `sum_vs_total_mismatch`, `article_not_in_text` — бейджи в табе «КП» (detail.blade). **Проверка на проде:** переразбор quote 3122 → 5→24 строки, артикул «ВВЕРХ» теперь M00878 (а не мисрид M00478=«Плата DOP-116»). Заявка тоже была неполной (Vision разобрал позиции из 2 из 6 фото). Остаток — двухколоночное «Кол-во» (см. «Текущие открытые вопросы»).
### Закрыто 2026-06-24
- ✅ **Эмодзи в PDF-экспорте переписки рендерились как «XX»** (`69619e0`). Кейс M-2026-5770: письмо поставщику с порталом («Открыть форму ответа» / «Кабинет поставщика») в PDF переписки показывало tofu-боксы «XX» вместо иконок 📝/🏢. Причина: тело письма лежит в БД и рендерится raw (`{!! $msg['body'] !!}`), а dompdf отрисовывает emoji через PT Sans, где нет emoji-глифов → `.notdef`. Фикс в `CorrespondenceExportService`: метод `stripInvisibleChars`→`stripUnrenderableChars` (2 call-site в `prepareBody`), regex расширен emoji-диапазонами (`\x{2600}-\x{27BF}\x{2B00}-\x{2BFF}\x{FE00}-\x{FE0F}\x{20E3}\x{1F000}-\x{1FAFF}`) — вырезается только иконка, текст кнопки остаётся; типографику (— · № «») не трогаем (проверено regex-тестом). Расширяет прежний кейс M-2026-5290 (zero-width tofu). **Письмо с порталом сгенерено НЕ текущим кодом MyLift** (`supplier-rfq.blade.php`/`supplier-rfq-catalog.blade.php` без токен-формы; строки «Открыть форму ответа»/«Кабинет поставщика» есть в репо только как коммент в Detail.php — вероятно LazyLift/legacy, отдельная задача на разбор источника). **Грабли деплоя (повтор owner=root):** целевой файл был owner=root от прошлого деплоя → `git pull` под www-data упал «unable to unlink … Permission denied»; лечение `chown -R www-data:www-data /var/www/mzcorp` + `git pull --ff-only`; деплой минимальный (нет миграций/composer/npm), `config:cache` + `queue:restart` (soft), 4 воркера RUNNING.

### Закрыто 2026-06-18
- ✅ **Фаза 3.3 — парсинг ответов поставщиков в офферы** (`cf2b636`). Foundation §4.4, focused-маппинг (не полный документ-парсер): знаем запрошенные позиции → LLM сопоставляет ответ. `supplier_offers` (inquiry×позиция×письмо: outcome quoted|refused, price/currency/valid_until_text/refusal_reason/raw_quote). `ParseSupplierReplyPrompt`+`SupplierOfferParser` (gpt-4o-mini): per-item quoted/refused/skipped из текста ответа (PDF/Excel-прайсы — фаза 3.3+); идемпотентно по (inquiry,message); статус позиций quoted/refused. `ParseSupplierReplyJob` (ShouldBeUnique по message_id) — диспатч из `SupplierInquiryService::attachMessage` при входящем ответе на запрос С позициями → после `AttentionService::onSupplierReplied`+activity SupplierReplied. UI: «Позиции и предложения» в Suppliers\Show. Проверено end-to-end на проде (Энкодер→quoted 4500 руб/5 дней, Ролик→refused «снято с производства»). Дальше 3.4 (supplier-pool) / 3.5 (напоминания).
- ✅ **Фаза 3.2 — формирование RFQ из заявки** (`971ec3f`, фикс `9fcfeb5`). Адаптация dispatch из LazyLift (`C:\Work\LazyLift` DispatchService — preset Message-ID→send→запись; LazyLift юзает токен/веб-форму, у нас email-ответы+matchInbound). Лёгкий статус (без pending_price_refresh; activity SupplierInquirySent гасит attention). `supplier_inquiry_items`(inquiry×позиция, status pending|quoted|refused|cancelled). `SupplierDispatchService`: preview (поставщик×позиции) + dispatch(req, supplierIds, itemIds, note, by) — на ВЫБРАННОГО поставщика письмо (шаблон emails/supplier-rfq) из ящика менеджера (createCompose+sendDraft) → SupplierInquiry(thread_root_id=msg_id, related_request_id=заявка)+items, исходящее уводим из клиентского треда в переписку поставщика. UI — **таб «Поставщики» карточки заявки** (commit 180cee4, по требованию; НЕ кнопка/модал; образец LazyLift SmartDispatchModal): `Requests\SupplierDispatchPanel` — (1) позиции с выделением неактуальных цен (catalogItem.is_price_actual=false, по умолчанию выбраны) + чекбоксы; (2) подобранные по матрице поставщики под выбранные позиции + ручное добавление любого из реестра (поиск), галочки кому слать (по умолчанию никто — широкие матрицы переподбирают, M-2026-5050→29); (3) превью письма + вложения (файлы заявки чекбоксами / с диска WithFileUploads). **Названия в письме — из каталога при сматченном M-SKU**, иначе клиентские; OEM=parsed_article. SupplierDispatchService::itemRows + dispatch(reqAttachmentIds,extraFiles копируются в черновик). Старый SupplierDispatchDialog+кнопка удалены. **Названия в превью РЕДАКТИРУЕМЫЕ** (input/позиция → editedNames → dispatch override) + **блок «Обращение»** с {поставщик} (personalGreeting подставляет имя персонально, нет→«коллеги») — commit 7232364. Прод: проверено (каталожные имена тянутся, неактуальные считаются, обращение подставляется); реальную отправку оператор делает через UI. Дальше 3.3 (парсинг ответов→SupplierOffer).
...
- **⏭ НАЧАТЬ ОТСЮДА: Vision путает две колонки «Кол-во» в КП** (открыт 2026-06-25). КП Liftway/МЗ имеют ДВЕ колонки «Кол-во»: «ожидается/заказано» и «к отгрузке/гарантировано». Сумма строки = `unit_price × ВТОРАЯ колонка`. `OutboundQuoteParsingService::parseWithGPT` иногда берёт ПЕРВУЮ → завышает `quantity`/`line_total`, Σ строк уезжает вверх от итога. **Канон:** `outbound_quotes.id=3122` (КП №358503, заявка M-2026-4755), поз.13 M24157: модель `qty=21,line_total=30122.82`, а в PDF «21,00/**1,00** шт», Сумма=1434.42 → завышение ровно 28688.40 = остаточный `sum_vs_total_mismatch` +6.1%. Любопытно: поз.24 (ТОТ ЖЕ M24157, «21,00/20,00») прочитан ВЕРНО (qty=20) — Vision непоследователен. **Не ловится `autoFixRowArithmetic`** (unit_price×qty==line_total внутренне сходится, ошибка только в выборе колонки); ловит лишь document-sum check. **Где чинить:** `buildParsingPrompt` — правило «при двух колонках Кол-во бери ту, при которой unit_price×qty≈Сумма строки (обычно к отгрузке)»; возможно усилить validateLineTotals/autoFix перебором колонок. **Риск регрессий** на одноколоночных/мерных — тестировать на нескольких КП. **Валидация:** `quotes:parse-outbound --apply --reset --quote=3122` → поз.13 qty=1/lt=1434.42, Σ line_total==472668.41, warnings пусты. Эталон — `pdftotext -layout` вложения att 22336. (Подробно в local-memory `project_dual_quantity_column.md` на машине автора.)
...
**Дополнение 2026-09-08:** пул `mzcorp-worker` (4 процесса, mail-sync первой) + `mzcorp-default-worker` (1 процесс, default первой) + `mzcorp-catalog-worker`. Приоритет очереди у воркера не вытесняет, а `mail-sync` с синком папок/флагов личных ящиков может не пустеть — поэтому один процесс всегда смотрит на default первой. Сторож: `queue:watchdog` (каждые 5 мин).
...
- **Postgres JIT съедает сотни мс на коротких запросах с раздутой оценкой** (2026-09-04). Признак в `EXPLAIN (ANALYZE)`: блок `JIT: … Timing: Optimization NNN ms, Emission NNN ms` сравним с Execution Time. Коррелированные подзапросы по jsonb-выражениям без индекса раздувают cost → JIT. Лечение: некоррелированная форма (NOT IN по множеству), индекс по выражению с partial-предикатом, повторяющим предикат запроса дословно; `ANALYZE` оценку per-row subplan не чинит.
- **Прод не ходит в GitHub — деплой через `git bundle`** (2026-09-02). На VPS `git fetch origin` падает `could not read Username for 'https://github.com'` (креды не сохранены, интерактива нет). Рабочая схема: локально `git push origin main`, затем `git bundle create /tmp/x.bundle <prod_head>..main` → `scp` → на проде `git fetch /tmp/x.bundle main:refs/deploy/incoming && git merge --ff-only refs/deploy/incoming && git update-ref refs/remotes/origin/main HEAD`. Последний `update-ref` обязателен, иначе прод считает себя «ahead of origin» и следующий деплой путается. **И НИКОГДА не делать `git checkout --` / `git restore` на проде, пока не убедился, что содержимое файла уже есть в коммите** — проверка `git diff <sha> --stat -- <files>` должна быть ПУСТА. На этом однажды потеряли правки, залитые на прод через scp в обход git.
- **На проде нет `php artisan test`** (2026-09-02). `composer install --no-dev` — phpunit отсутствует, тесты гоняются только локально. Синтаксис правок проверяется `scp` в `/tmp` + `php -l`. Логику чистых функций (regex-парсеры и т.п.) удобно валидировать одноразовым скриптом на проде против реальных данных. **Осторожно: `extract` — builtin PHP**, функцию с таким именем в скрипте объявить нельзя (`Cannot redeclare extract()`).
- **Тяжёлые tinker-скрипты по `detected_artifacts` (jsonb) вешают ssh на минуты** (2026-09-02). Eloquent-выборка с гидрацией моделей по 30-дневному окну не укладывается в разумное время (Beget Cloud DB удалённая). Писать чистый SQL через `DB::select` и держать окно ≤14 дней; имена пользователей подтягивать одним `User::pluck('name','id')`, а не relation'ом в цикле.
...
### Сессия 2026-09-08 — Почта: папки и подпапки, выделение писем и bulk-действия, двусторонний \Seen с владельцем ящика; переход из заявки в почту

**Задачи заказчика:** (1) из карточки заявки открыть почту со всеми письмами заявки; (2) «как в Яндексе»: папки/подпапки, Ctrl+A выделяет письма, а не текст, пометить прочитанными, drag&drop в папку; (3) прочитанность синхронна с сервером, но **только для менеджера в своём ящике** — директор, заглянувший в чужой ящик, ничего не меняет.

#### Решения

| Вопрос | Решение | Почему |
|---|---|---|
| Где живут папки | В mzCorp (`mailbox_folders`, `email_messages.mailbox_folder_id`), письма в Яндексе остаются в INBOX | Yandex IMAP не даёт EXPUNGE после UID MOVE/COPY — физический перенос оставляет дубли (см. «Известные грабли»); папки нужны менеджерам внутри CRM, а не в Яндексе |
| Глубина/имена | `MailboxFolder::MAX_DEPTH=3`, имя ≤80, уникально в пределах родителя; удаление папки — письма → «Входящие», подпапки → к родителю | Не терять письма при удалении |
| Кому можно | Папки видны всем, у кого есть доступ к ящику (`accessibleMailboxIds`); счётчик непрочитанных — per-user | Общие ящики читают несколько человек |
| Выделение | Alpine-state в pane списка: `sel` Set, Shift-диапазон, Ctrl+A (игнорируя редактируемые поля), Esc; bulk-бар `Прочитано / Непрочитано / В папку ▾ / Все на странице`; `draggable` строки, ghost с числом писем; drop-цели — кастомные папки и «Входящие» (= убрать из папки) | Как в Яндекс.Почте |
| \Seen push | `ImapSeenSyncService::pushSeen(ids, user, seen)` → `PushImapSeenFlagJob` на (ящик, папка) — только если `isOwner(mailbox,user)` (личный ящик + `owner_user_id`) | Требование заказчика: директор в чужом ящике не влияет |
| \Seen pull | `PullImapSeenFlagsJob` из `mail:sync` для личных ящиков с владельцем: FETCH FLAGS по INBOX за 14 дней; \Seen → `markManyRead` владельца; не \Seen и наше `read_at` старше 10 мин → снять прочитанность. Лог пишется только если что-то изменилось | Грейс 10 мин, чтобы наш push не откатился pull'ом до того, как сервер применит флаг |
| Правило 8 CLAUDE.md | Добавлено «Исключение 2»: \Seen ставится/снимается только по действию владельца личного ящика | Иначе противоречие с «никогда не ставим \Seen» |

#### Проверка на проде
- tinker на ящике 14 (Якубович): `storeSeen(true)` → флаги `["\Seen","encrypted"]`, `storeSeen(false)` → обратно; `pullSeen` → 373 прочитано / 0 снято за 0.5 с.
- Браузер (info@, mbox=15): папка «Тест папка» → чекбокс → «В папку ▾» → письмо в папке (`f:1`, «1 писем») → `deleteFolder` → письмо снова во «Входящих», кастомных папок 0. Тестовых данных не осталось.
- Хвост UI: bulk-бар переносился в узкой панели, точка «непрочитано» накладывалась на чекбокс — `9cf7516`.

**Не сделано:** см. «Открыто (2026-09-08)».

#### Yandex IMAP: что выяснилось при синке папок (2026-09-08)
- `UID SEARCH` (любой критерий) → `NO [UNAVAILABLE] UID SEARCH Backend error`. Искать по Message-ID нельзя; только `FETCH 1:* (UID)` (`getUid()`), 12 с на INBOX в 120k — для INBOX не применять, только для пользовательских папок.
- `UID MOVE` работает (capability MOVE, в ответе `COPYUID <validity> <src> <dst>` + `EXPUNGE`) — в т.ч. в папки с кириллицей (MUTF-7 путь как в LIST). Старая заметка «Yandex не чистит после MOVE» относилась к COPY+STORE+EXPUNGE в `MailFolderRouter`.
...
#### Статусы: ложное «ждёт счёт» от роутера цитируемого КП (M-2026-12166, `5704d4b`, `3750cb4`)

Клиент ответил «а с резьбой М10 есть?» на наше напоминание по КП; в цитате были номер 364274 и «перейти к выставлению счёта». `InboundReplyLinker` не привязал (заявка авто-закрыта, Guard A), `CitedOutboundQuoteRouter` нашёл номер+ключевые слова в НАШЕЙ цитате → reanimate + `awaiting_invoice`; следом реальный интент клиента `under_review` был отклонён («запрещённый переход awaiting_invoice → under_review»). **Фикс:** роутер по-прежнему ищет номер КП во всём тексте (для привязки письма к заявке КП), но `invoice_intent` берёт только из собственного текста клиента (`ownBodyText` через `cutQuotedReplyTail`); `MailRouter::applyCitedInvoiceRequest` без интента привязывает/реанимирует, статус не трогает. Регекс интента с lookbehind `(?<!\p{L})` — «Насчет оригинала» не запрос счёта. Cleaner: cut от первой «>»-строки, если цитата преобладает (mail.ru ставит подпись клиента ПОСЛЕ цитаты — хвостовое правило не срабатывало) + атрибуция mail.ru «Понедельник, … от X <x@y>:». Переоценка за 14 дней (8 заявок в «ждёт счёт» без счёта от body-маршрута): 4 откачены в quoted (M-2026-4726 «прошу обновить КП», 9877 «сможете поставить?», 12166, 13471 «уточните»), 4 оставлены (реальные просьбы о счёте). M-2026-12166 → under_review вручную по письму клиента.

#### Поставщики: сотрудники стали «поставщиками» через ящики снабжения (`668223b`)

Вопрос заказчика «почему внутренняя переписка (Боев → Соколова, CC Курзаев) в почте Курзаева с меткой „поставщик“». Ответ на первую часть: Курзаев в CC, письма реально в его ящике. Метка — баг: `Mailbox::isProcurementMailbox()` (Петрова, Милючихин) → `ingestSupplierMessage` для ЛЮБОЙ входящей → коллега, написавший в снабжение, становился `supplier_inquiries.supplier_email=*@myzip.ru`; дальше домен-фолбэки `matchInboundBySubject`/`matchByCodeInSubject` тянули к этим inquiry любую переписку с @myzip.ru во всех ящиках. Было 16 таких inquiry (Боев, Роденков, Курзаев, Якубович, info@, noreply@…), 326 писем. **Фикс:** `ingestSupplierMessage` → `?SupplierInquiry`, для внутреннего отправителя (`InternalSenderDetector::detect`) возвращает null и ставит category=irrelevant; домен-фолбэки игнорируют internal_domains и FREE_MAIL_DOMAINS. **Данные:** 16 inquiry удалены (items/offers = 0), 326 писем отвязаны, 273 письма сотрудников перекатегоризованы supplier_reply → irrelevant, 53 письма внешних адресов отвязаны без смены категории.
...
| **Invoiced / Quoted только по факту документа** | `DetectorType::requiresDocumentEvidence()` (outbound_invoice, outbound_quotation_full/partial): `AiDecisionService::recordSuggestion` больше не auto-apply'ит сразу — suggestion пишется, статус двигает `ParseOutboundQuoteJob` через `AiDecisionService::applyAfterDocumentEvidence`: **счёт — только когда `InvoiceService::autoIssueFromOutboundQuote` создал Invoice** (требует document_number; parse даёт позиции и сумму); **КП — только когда OutboundQuote распарсен с ≥1 позицией** (по требованию заказчика: «КП легитимно только при наличии реального КП с позициями»). Match-boost для этих типов отключён (совпадение позиций — не доказательство счёта, реквизиты/спецификация тоже содержат номенклатуру). Нет вложения / скан / HTML-only КП без файла / не тот документ → suggestion остаётся на плашке менеджеру. Ручные счета и КП из mzCorp (`InvoiceService` → Invoiced, `OutboundReplyHooks::handleQuotationSentHook` → Quoted) и триаж непривязанных не затронуты — там документ есть по построению. `RequestStatusReassessor::hasOutboundQuote` ужесточён до КП с позициями. |
...
#### Follow-up в той же сессии: «перестали автоматически применяться КП» (M-2026-14815)

Жалоба заказчика после деплоя гейта. Факты: M-2026-14815 применилась автоматически через 18 с после отправки (12:36:14 → 12:36:32), но за день 8 КП + 5 счетов были `manually_confirmed` через **2–20 секунд** после создания решения — менеджеры (или они же на открытой карточке) жали плашку «AI: отправлено КП — применить», пока парсер ещё работал. Часть из них — `SendClientDocumentDialog` (диалог «Отправить документ клиенту»): он сам делает `recordSuggestion(1.0)` + `apply` с пользователем, это штатный детерминированный путь (менеджер явно положил файл в слот «КП»), он не ждёт парсер. **Фикс:** для типов requiresDocumentEvidence при наличии парсимого вложения suggestion получает `payload.awaiting_document_parse_until = now+15 мин` (`services.quotes.awaiting_parse_minutes`); `Detail::pendingAiDecisions` фильтрует через `AiDecision::scopeActionable()`; `ParseOutboundQuoteJob` в `finally` зовёт `AiDecisionService::markDocumentParseFinished($message)` — снимает ключ с оставшихся suggested (нет позиций / не счёт / ошибка) → плашка появляется только когда автоматика реально не смогла. Дедлайн — страховка от потерянной job'ы. Ограничение: несколько вложений в письме — первый завершившийся разбор снимает ключ, плашка может мигнуть до второго.
...
- **Auto-mode на проде включён** для outbound_invoice / quotation_full / clarification / declined (`detector.auto_mode.*` = true, порог по умолчанию 0.85). После этой сессии invoice и quotation auto-apply'ятся только из парсера (см. таблицу), clarification/declined — по-прежнему по письму (там документа нет по определению).
- **Принцип заказчика для вех (2026-09-07):** «Счёт отправлен» и «КП отправлено» легитимны только при распознанном документе (счёт: номер+позиции+сумма; КП: позиции). Детектор по тексту письма и имени файла — только подсказка.
...
**Контекст:** день начался с проверки после сбоя OpenAI, дальше — цепочка задач заказчика: аналитика исходящей почты и IQOT, фикс классификатора, фикс видимости писем в «Почте», и главное — **новый модуль «Рекламные блоки в письмах»**. Всё задеплоено (последний коммит `ed32d63`), прод = origin/main.

#### Что задеплоено

| Модуль | Что | Коммиты |
|---|---|---|
| **Реклама в письмах** (новый) | `marketing_blocks` (картинка + заголовок + текст + ссылка, is_active, impressions_count, last_used_at). Админка `/dashboard/marketing-blocks` (`Admin\Marketing\Blocks`, роли director/admin; пункт «Реклама» в топбаре + карточка в Настройках). Живое превью в iframe, тестовая отправка с выбранного ящика на адрес (без сохранения блока). Вставка в `OutgoingMailMimeBuilder::composeFinalBody()` — ручные ответы + авто-уведомления; исключения: поставщики (`SupplierRegistry`/`supplier_inquiry_id`) и внутренние адреса. Случайный активный, один на письмо (id в `detected_artifacts.marketing_block_id`), счётчик после успешной отправки. Картинка на private-диске `marketing/`, в письме CID (как логотип), в CRM — auth-роут `marketing-blocks.image`. Глобальная настройка `marketing.block_position` (над/под подписью). Плашка на всю ширину. Гайд `resources/docs/director/marketing-blocks.md`. | `548322b` `8185074` `0c09dea` `572824a` `4876411` `ed32d63` |
| **Настройки: форма не отправлялась** | `<input type=number step=0.5>` у курса USD, значение ЦБ 86,8872 не кратно → браузер блокировал submit ВСЕЙ формы. Float-поля → `step="any"`. | `b9e07c1` |
| **Классификатор: ссылка на счёт ≠ просьба счёта** | Пост-LLM override post_sale→client_request ловил реквизит «Счёт на оплату № 3228 от 25 марта» как запрос счёта → заявка-фантом M-2026-14524 (Liftway, «сообщите дату отгрузки»). `looksLikeInvoiceRequest`: сначала глагол+«счёт» (+ «обновить»), затем вырезаем `INVOICE_DOC_REF_RE`, потом голое «счёт на оплату». `DELIVERY_STATUS_RE` += «дата отгрузки/отправки». Проверено на 154 override за 30 дней: 143 без изменений, 11 → post_sale (все постпродажа). +6 тестов. | `c4cf07a` |
| **«Почта»: письмо не видно в info@** | Многоадресное письмо (To: info@, Andrey.Vasukhno@) лежит в двух ящиках; «оригиналом» стал личный (синк раньше на ~40 с), запись в info@ получила `cross_mailbox_copy_of` и пряталась безусловно. Теперь: один выбранный ящик — без фильтра; несколько — NOT IN множества «скрыть» (оригинал или более ранняя копия в выборке). Частичный индекс `email_messages_cross_mailbox_copy_of_idx`. | `4ba5bc2` `364b676` |

#### Аналитика, выданная заказчику (без кода)

- **Исходящая почта через MyLift** (наша подпись), клиентские письма: ~300/раб.день за 30 дней, 67% авто (order_received ~105/день), ~670 уникальных контактов/месяц, ~1 200/квартал. 39 контактов получают 59% писем (liftway.store — 1 адрес, 1 091 письмо/мес). Прямых из Yandex столько же, но доля MyLift растёт (ручные через MyLift 0 → 600–700/нед за лето).
- **IQOT топ-20** позиций с лучшей ценой на рынке (status=completed, ≥3 конкурентов, место 1–2) и топ-20 в наличии с неактуальной ценой — CSV отданы. **Находка:** IQOT включает наше же предложение «MyLift» как участника рынка (10 отчётов) — `priceComparison()`/бейджи считают его конкурентом (M03035 показан 2-м с Δ0% вместо 1-го с −12%). Не чинил.
- **Сбой OpenAI 03.09** — реально окно 09:36–09:37 (прокси `ai3.lazylift.ru:8443`), 3 разбора КП упали и самовосстановились кроном `quotes:reparse-failed` в 10:00. Писем не потеряно.

#### Заметки / уроки

- **Postgres JIT на проде включён** и бьёт по коротким запросам с завышенной оценкой: коррелированный NOT EXISTS по jsonb дал cost 500k+ → JIT 420 мс на 30-мс запрос (EXPLAIN ANALYZE: Optimization 254 + Emission 146 мс). `ANALYZE` не помог. Обошёл переписыванием предиката (некоррелированный NOT IN). Глобально `jit=off` не трогал — отдельное решение.
- **Проверочные скрипты на проде обязаны чистить за собой даже при падении.** Первый прогон `pos_check` упал на `BindingResolutionException` (угадал не то имя класса) ПОСЛЕ создания тестового блока «__POS» — он остался в проде (неактивен, 0 показов, удалён вручную). И **никогда не создавать каталоги в `storage/` из-под root**: `storage/app/private/marketing` от root:700 → php-fpm не писал, а Livewire `storeAs()` вернул путь без файла. См. «Известные грабли».
- Symfony `embedFromPath($path, $cid)` переименовывает CID в `<hex@symfony>` и подменяет `cid:NAME` в html при сборке — проверять встраивание по `Content-ID`/`inline; name=…`, а не по `cid:mylift-promo` в сыром MIME.
- `MailboxAccessService`: у делегата (RequestDelegation) доступ к ящику отпускника; `sweepOverdue` шлёт SLA-уведомление acting'у; `DelegatedRequestNotifier` — in-app+email на письмо клиента (троттлинг 30 мин). Делегирование не переназначает — так задумано (подтверждено заказчиком 04.09).

### Сессия 2026-09-02 — Платёжки не плодят заявки-фантомы + доставка письма назначенному, когда владелец ящика в отпуске

**Контекст:** две независимые правки в mail-pipeline, обе задеплоены и провалидированы на реальных прод-данных. Плюс ранняя часть сессии — UI-фикс пустых писем в разделе «Почта» и автообновление списка.

#### Что задеплоено

| Модуль | Что | Коммит |
|---|---|---|
| **Платёжное поручение ≠ новая заявка** | `PaymentDocumentDetector` — вердикт LLM по вложению (`doc_type`) + номер счёта из текста PDF пишутся в `email_messages.detected_artifacts.payment_document`; гард в `ParseRequestItemsJob` (ветка empty-items, ДО `assignIfStuckPending`) гасит заявку-фантом. Нашли счёт → письмо переезжает на заявку счёта + `closed_lost/duplicate` + attention 🛒; не нашли → `closed_lost/post_sale_correspondence`. | `8429f37`, фикс `f8acf9b` |
| **Доставка письма при отпуске владельца ящика** | Guard `origin_in_another_personal` в `MailDeliverToManagerService` пропускает APPEND, если владелец origin-ящика `isUnavailable()` или архивирован. | `f75b19d` |

#### 1. Платёжка без треда → заявка-фантом (кейс M-2026-14395)

**Разбор кейса.** Письмо #95311, 02.09 17:02, `h.tamik1@mail.ru` → ящик 15 (`info@myzip.ru`, shared). Мобильная Почта Mail кладёт имя файла в тему: subject = `b094243a-….pdf`, тело 34 символа, одно PDF 186 КБ, **ни In-Reply-To, ни References, ни `Re:`**. `CategorizeIncomingPrompt` ПРАВИЛО 3 («пустое тело + PDF → client_request, имя вложения значения не имеет») даёт client_request 0.85 — содержимое вложения классификатор не видит в принципе, поэтому ПРАВИЛО 11 (платёжка → post_sale) недостижимо. Линкеру зацепиться не за что — треда нет. Парсер PDF прочитал, `classifyHeavyAttachmentRelevance` вернул «платежное поручение», вложение отбросил, позиций 0 → через 8 секунд авто-ответ клиенту + назначение менеджера.

Совпадающий счёт существовал всё это время: `invoices#3200`, `invoice_number "8772"`, 16 633,18 ₽, статус `expired`, `request_id 12879` = **M-2026-12878** (`closed_lost/invoice_unpaid`). Единственная связь — строка «Счет на оплату № 8772» ВНУТРИ платёжки.

**Проверка на регресс (по требованию заказчика — «раньше подобные письма не создавали пустышки»).** За 14 дней из 23 писем с платёжными документами 18 корректно прицепились к существующим заявкам (это `Re:`/`Fwd:` в живых тредах — линкер работает), и ровно 5 без тредовых заголовков породили фантомы. Регрессии нет, дыра узкая — «платёжка без треда». Изначальная идея «гасить любую заявку с 0 позиций» отвергнута: из тех 5 фантомов 3 стали `closed_won`.

**Правило заказчика:** «если определили что это платежное поручение, но не смогли найти счёт — помечать как постпродажная переписка». Отсюда `ClosedLostReason::PostSaleCorrespondence`.

**Два бага, вскрывшихся ТОЛЬКО на реальном прогоне** (`dispatch_sync(new ParseRequestItemsJob(95311, force: true))`):
1. `invoice_number: null`. Первая версия `extractInvoiceNumber` резала текст по подписи «Назначение платежа». В извлечённом из PDF потоке порядок блоков формы НЕ совпадает с визуальным — **подпись поля идёт ПОСЛЕ самого назначения**, окно отрезало строку со счётом. Окно убрано полностью. Реквизитный мусор отсекается иначе: банковские счета в платёжке пишутся сокращённо («Сч. № 4070…»), а regex требует полное «счет/счёт»; собственный номер поручения идёт как «ПЛАТЕЖНОЕ ПОРУЧЕНИЕ № 36» — без слова «счёт»; страховка — кандидаты длиннее 8 цифр режутся (р/с 20, ИНН, БИК).
2. **Гард не срабатывал** — `handleEmptyRequest` читал `detected_artifacts` со stale-инстанса `$message`, загруженного джобом ДО парсинга, а `record()` сохраняет через отдельно загруженную модель. Лечение — `$message->refresh()` первой строкой.

**Результат прогона после фиксов:** msg#95311 → `category=post_sale`, `related_request_id=12879`, артефакт `{"doc_type":"платежное поручение","invoice_number":"8772"}`; M-2026-14395 → `closed_lost/duplicate` с пояснением; M-2026-12878 → `attention="post_sale"`.

**Остальные 4 исторических фантома ремедиации не требуют:** M-2026-12866/12868/12869 → `closed_won`; M-2026-13578 → уже `closed_lost/duplicate`.

**Тест:** `tests/Unit/Services/Mail/PaymentDocumentDetectorTest.php` — фикстура `REAL_PAYMENT_ORDER` это ДОСЛОВНЫЙ текст, извлечённый из реального PDF (с подписью поля ПОСЛЕ назначения). 5 кейсов: извлечь 8772; игнорировать р/с и `Счет № 40702810902540000757`; игнорировать собственный № 36; извлечь `НФ00-005513`; null без ссылки на счёт.

**Заметки по реализации:**
- `record()` вызывается из `RequestItemParsingService::classifyHeavyAttachmentRelevance` — там уже есть и `doc_type` от LLM, и извлечённый текст, второй раз читать файл не нужно. Держим `$fullText` для детектора, а в LLM отдаём урезанный `$textHead` (1500 симв.).
- Запись артефакта НЕ зависит от `is_nomenclature` — флаг ненадёжен (msg #82705 получил `true` на платёжке).
- `resolveInvoice` обязательно гардит по клиенту (`lower(client_email)` заявки счёта): номера счетов не уникальны глобально, 1С нумерует по-своему.
- `AttentionService::onPostSaleMessage` расширен на `RequestStatus::ClosedLost` — заявка счёта чаще всего уже закрыта. Безопасно: остальные вызывающие (`MailRouter::resolvePostSaleRequest`, `PostSaleRerouteService::validate`) фильтруют closed_lost сами.

**Открытые хвосты (НЕ сделано намеренно):** счёт 3200 остаётся `expired`/неоплаченным — оплату записывает только импорт из 1С (`markPaid` сработает, `expired ∈ PAYABLE_STATUSES`), помечать оплату по картинке из PDF нельзя. Отдельный вопрос к бизнесу: срок действия счёта 5 дней, а оплата пришла на 5-й день после истечения.

#### 2. Письмо-инициатор не доходило до назначенного, когда владелец ящика в отпуске

**Вопрос заказчика:** «менеджер недоступен → заявки уходят активным, но само письмо назначенному не присылается — не срабатывает или не заложено?»

**Ответ: заложено, но глушилось.** `AssignmentService::autoAssign` (стр. 169-176) всегда диспатчит `DeliverToManagerInboxJob`. Обрыв — в `MailDeliverToManagerService`, guard `origin_in_another_personal`: письмо лежит в личном ящике другого менеджера → skip. Посылка guard'а («владелец A видит письмо у себя в Yandex и работает с ним, B хватит треда в UI») для отпуска неверна — A в отпуске, письмо никто не откроет. При этом `pickStickyByDirectMailbox` намеренно НЕ садит заявку на недоступного владельца, она уходит доступному коллеге — и тот получает заявку без письма, ответить клиенту обычным reply из своего ящика не может.

**Замер до правки:** заявки из личных ящиков двух отпускников (Якубович до 11.09, Васюхно до 07.09), назначенные другим, за 14 дней — **63 из 63 без письма**. В `mail_append_audit` — `skipped/origin_in_another_personal`.

**Правка:** в guard добавлено `! $originOwnerOutOfPlay` (владелец архивирован ИЛИ `isUnavailable()`). Остальные защиты (идемпотентность по `inbox_deliveries[]`, «уже в ящике», «нет личного ящика») не тронуты.

**Бэкфилл** — отдельная команда не нужна, `mail:backfill-manager-deliveries` (крон каждые 30 мин) сама зовёт `deliver()`, её просто глушил тот же guard. `--apply` дал: `доставлено=35 · уже_было=906 · уже_в_ящике=286 · silent skip=16`. Все 16 пропусков проверены — ящики **доступных** владельцев (Румянцев, Агрызков), т.е. исходный кейс guard'а продолжает работать.

**Уточнение заказчика по делегированию (ничего не менял, так и задумано):** пометка отпуска НЕ переназначает уже существующие заявки — `ManagerUnavailabilityService::delegateActiveRequests()` создаёт `RequestDelegation` (`acting_user_id`), а `assigned_user_id` остаётся за отпускником. Делегат получает только уведомления о событиях, требующих внимания. Проверено, что механизм работает: `DelegatedRequestNotifier::notifyActingManagers` (зовётся из `MailRouter.php:758` на реальном письме клиента, троттлинг 30 мин на пару acting×request) шлёт in-app + email; `AttentionService::sweepOverdue` (стр. 388-389) при SLA-просрочке явно выбирает `activeDelegations->first()?->actingUser ?? assignedUser`; `Pool.php:656-678` показывает делегату заявку в его пуле; `MailboxAccessService::delegatedOriginalMailboxIds` открывает ему ящик отпускника в разделе «Почта».

#### Грабли деплоя (повтор, зафиксировать)

- **Прод не ходит в GitHub** (`could not read Username for 'https://github.com'`). Рабочий транспорт — `git bundle`: `git bundle create /tmp/x.bundle <prod_head>..main` → scp → на проде `git fetch /tmp/x.bundle main:refs/deploy/incoming && git merge --ff-only refs/deploy/incoming && git update-ref refs/remotes/origin/main HEAD`.
- **НИКОГДА не делать `git checkout --` на проде, пока не убедился, что содержимое файла уже есть в коммите.** Проверка: `git diff <sha> --stat -- <files>` должен быть ПУСТ. (В прошлой сессии на этом потеряли правки.)
- **На проде нет `php artisan test`** (`composer install --no-dev`, phpunit отсутствует). Линт правок — scp в `/tmp` + `php -l`. Логику regex проверял отдельным скриптом на проде против реального текста PDF. Осторожно: `extract` — builtin PHP, нельзя объявлять функцию с таким именем в одноразовых скриптах.
- **Локально PHP нет вообще** (Windows-машина без Herd в этой сессии) — вся проверка через `ssh mzcorp`.
- Тяжёлые tinker-скрипты по `detected_artifacts` (jsonb) за 30 дней вешают ssh-сессию на минуты. Писать чистый SQL через `DB::select` с окном ≤14 дней.
