...
- **M-2026-1495 — парсер режет параметры товара на отдельные позиции** (открыт 2026-05-24). Письмо `a.tulupov2014@yandex.ru`: «Жилье, одиночная, нерегулируемый. + фото платы». Это **одна** позиция (плата управления с параметрами: для жилья, режим «одиночная работа», «нерегулируемый привод»). Парсер создал **3 позиции** «Жилье», «Одиночная», «Нерегулируемый» — последние две даже сматчились на разные платы в каталоге (M08145 «Плата управления ПУ-1 одиночная нерегулируемый» и M06023 «Плата ПУ-3 нерегулируемый»). Промпт `ParseItemsPrompt` не распознал, что три слова — это параметры одного товара, а не три отдельные позиции. Требует доработки промпта (явное правило: «короткие списки прилагательных/режимов рядом = параметры одной позиции, а не разные») и/или Vision-проверки на наличие фото платы.
