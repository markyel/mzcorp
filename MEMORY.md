...
### Открыто (2026-09-08)
- ⏳ **Воркеры очереди могут зависнуть внутри IMAP-задачи и не реагировать на `queue:restart`** (3 воркера с 14:36 до 16:12, 2026-09-08). Что именно висело — не установлено (кандидаты: PullImapSeenFlagsJob/PushImapSeenFlagJob/SyncImapFoldersJob при перегрузке Yandex параллельными сессиями + backfill). Идеи: alert на `jobs.created_at` старше 10 мин в `default`; отдельный воркер только для `default`; `--timeout` воркера и проверка, что SIGALRM реально убивает процесс при блокирующем чтении.
