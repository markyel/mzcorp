...
- **docs-bot (user id 19, docs-bot@myzip.ru) — НЕ должен быть в active+available** (2026-05-27): для скриптов скриншотов через puppeteer (`scripts/capture-docs-screenshots.mjs`) создан служебный user с role `head_of_sales`. Эта роль входит в `Role::requestHandlerRoles()` — `AssignmentService::autoAssign` начал round-robin'ить новые заявки на него (10 штук прилетело за полчаса). Лекарство: `users.unavailable_until = 2099-12-31` — `User::scopeAvailable` исключает его из distribution навсегда, но login для скринов работает. При повторном создании похожих служебных users — сразу выставлять unavailable_until=2099 или флаг `archived_at`.
