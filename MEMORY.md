...
| §ext-codes-cli | **CLI-инструменты для исторических данных.** (1) `mail:reassign-by-external-code` — backfill: проходит по всем email_messages с LZ-REQ-маркером, находит «правильного родителя» через тот же алгоритм `matchByExternalCode` и перепривязывает неправильно связанные. Запись audit в `email_messages.detected_artifacts` (`type: manual_reassign_by_external_code, from_request_id, to_request_id, codes, reassigned_at`). Опции: `--apply` (default dry-run), `--limit=N`. (2) `mail:detect-duplicate-requests` — отчёт: ищет случаи где один LZ-REQ-маркер привязан к ≥2 разным Request (LZ-REQ-1208 → 4 Request, LZ-REQ-1129 → 4 Request, etc.). Группирует, показывает status / assigned / email_count per Request. Опция `--csv=path` для CSV-дампа. Только отчёт — слияние Request делается вручную через UI. | новое | done |
