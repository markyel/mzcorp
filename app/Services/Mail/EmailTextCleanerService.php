...
class EmailTextCleanerService
{
    ...
    /**
     * Срезать ЛЮБУЮ цитату ответа (не только нашу) — для ИСХОДЯЩИХ писем
     * менеджера, где ниже его текста процитировано письмо клиента. Оставляем
     * только собственный текст автора. Маркеры конца собственного текста:
     *   - атрибуция «…, X <x@y> написал(а):» / «… пишет:» / «… wrote:»;
     *   - «-----Original Message-----» / «-----Исходное сообщение-----»;
     *   - Outlook-заголовок цитаты: строка «От:/From:» и в ближайших 4 строках
     *     «Отправлено:/Sent:» или «Кому:/To:» (без «>»-префиксов);
     *   - хвостовой блок строк с «>»-префиксом до конца письма (inline-ответы
     *     между цитатами сохраняются: режем только ХВОСТ).
     *
     * Кейс M-2026-14608: LLM-классификатор исходящих читал «Прошу выставить
     * счёт» из цитаты клиента как наш счёт. dequoteText для этого не годится —
     * он сознательно СОХРАНЯЕТ содержимое цитат (нужно inbound-парсеру).
     *
     * Если собственного текста нет (весь ответ — цитата) — возвращаем ''.
     */
    public function cutQuotedReplyTail(string $text): string
    {
        if (trim($text) === '') {
            return '';
        }
        ...
        foreach ($lines as $i => $line) {
            ...
            if (preg_match('/^-{4,}\s*(?:Пересылаемое сообщение|Пересланное сообщение|Forwarded message|Перенаправленное сообщение)/iu', $t) === 1) {
                $inForward = true;
            }
            // Яндекс: атрибуция без «пишет» — «10:01, 25 сентября 2026 г.,
            // "Владимир Головнёв" <v@myzip.ru>:», над ней «Кому: …;», «Тема: …;»
            // и разделитель «----------------». Цитата нашего письма считалась
            // текстом клиента: «Вы оплатили счёт № 10061… доплатите доставку?»
            // (M-2026-17264), «Мы отправляли 16-го… Счет на Родник» (M-2026-16693).
            // Внутри пересылки не режем: там так же выглядит шапка пересланной
            // заявки коллеги клиента, а её позиции нужны.
            if (! $inForward && mb_strlen($t) <= 300
                && preg_match('/<[^<>\s@]+@[^<>\s]+>\s*:\s*$/u', $t) === 1
                && preg_match('/\d{1,2}[.:]\d{2}/u', $t) === 1) {
                $cutAt = $i;
                while ($cutAt > 0) {
                    $prev = trim($lines[$cutAt - 1]);
                    if ($prev === '' || preg_match('/^(Кому|To|Копия|Cc|Тема|Subject)\s*:|^-{4,}$/iu', $prev) === 1) {
                        $cutAt--;
            ...
            if (mb_strlen($t) <= 300
                ...
                && preg_match('/@|\d{1,2}[.:]\d{2}|\d{4}/u', $t) === 1) {
                $cutAt = $i;
                break;
            }
            ...
            if (preg_match('/^(От|From)\s*:/iu', $t) === 1) {
                for ($j = $i + 1, $cap = min($count, $i + 5); $j < $cap; $j++) {
                    if (preg_match('/^(Отправлено|Sent|Кому|To)\s*:/iu', trim($lines[$j])) === 1) {
                        $cutAt = $i;
                        break 2;
                    }
                }
