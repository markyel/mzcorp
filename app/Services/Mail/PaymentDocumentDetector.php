...
class PaymentDocumentDetector
{
    ...
    /**
     * Вытащить номер счёта из текста платёжного поручения.
     *
     * Ищем в «назначении платежа» — там живёт ссылка на счёт («Счет на оплату
     * № 8772 от 21 августа 2026. Контроллер эскалатора…»). Реквизитная часть
     * платёжки полна номеров (р/с, к/с, БИК, ИНН) и собственного номера
     * поручения («ПЛАТЕЖНОЕ ПОРУЧЕНИЕ № 36») — по ним матчиться нельзя.
     */
    public function extractInvoiceNumber(string $text): ?string
    {
        $haystack = $text;
        if (preg_match('/назначени\w*\s+платеж\w*/iu', $text, $m, PREG_OFFSET_CAPTURE) === 1) {
            $haystack = mb_substr(substr($text, $m[0][1]), 0, 600);
        }
