...
class PostSaleFulfillmentDetector
{
    ...
    /**
     * Просьба выставить счёт НА ПЕРЕЧЕНЬ позиций — признак заказа.
     *
     * Требуем и глагол выставления рядом со счётом, и указание на позиции
     * («по следующим позициям», «на позиции», «по списку», «по спецификации»):
     * без второго условия под правило попадает «сделайте счёт по заказу» — это
     * уже оформленная сделка.
     */
    private const INVOICE_FOR_ITEMS_RE = '/выстав\w*\s+сч[её]т\w*[^.!?\n]{0,80}(следующ\w+\s+позици|позици|списк|спецификац|наименовани)|(следующ\w+\s+позици|позици|списк|спецификац)\w*[^.!?\n]{0,80}выстав\w*\s+сч[её]т/iu';

    /** Reply-контекст: вопрос о сроках/доставке уже размещённого заказа. */
    // «Мы доплатим доставку» — ответ на вопрос менеджера по уже оплаченному
    // счёту (самовывоз или доставка), а не новый заказ: «оплатим» внутри
    // «доплатим» уводило письмо в дочернюю «Новый счёт» (M-2026-17264).
    // «Может в Деловые Линии успеете сегодня отдать?» (письмо 96857) — тоже
    // отгрузка: раньше это «угадывалось» только по нашей цитате ниже.
    ...
    public function deliveryStatusInquiry(EmailMessage $message): bool
    {
        ...
        // «Выставите счёт на эти позиции» — это заказ, а не вопрос о статусе,
        // даже если рядом стоят «сроки поставки». Письмо Liftway «Просим
        // подтвердить наличие, цены и сроки поставки по следующим позициям и
        // выставить счёт» уезжало в постпродажу из-за reply-ветки ниже: слова
        // про сроки есть, а заказ на две позиции терялся вместе с заявкой.
        //
        // Гард узкий и намеренно не совпадает с requestsInvoiceToPay: тот
        // считает просьбой о счёте и «сделайте счёт по заказу во вложении», и
        // «по срокам успеваем? Счет № 6272» — а это как раз постпродажа
        // (проверено прогоном корпуса: широкий гард ломал 4 письма из 542).
        // Здесь нужен глагол выставления рядом со счётом и перечень позиций.
        if (preg_match(self::INVOICE_FOR_ITEMS_RE, $haystack) === 1) {
            return false;
        }
