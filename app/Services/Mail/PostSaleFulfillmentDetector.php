...
class PostSaleFulfillmentDetector
{
    ...
    private const DELIVERY_STATUS_RE = '/отправили\s+(ли\s+)?(нам\s+)?(заказ|товар|посылк)|отгрузили\s+ли|дат\w*\s+(поступлени|отгрузк|отправк|готовност|прибыти)|готовност\w*\s+заказ|статус\s+(заказа|отгрузк\w+|поставк\w+|доставк\w+)|заказ\s+готов|готов\s+ли\s+заказ|готов\w*\s+к\s+выдаче|получен\w*\s+(ли\s+)?оплат|возможн\w+\s+забрать|забрать\s+(товар|заказ|со\s+склад)|забер[её]м|возврат\s+(по\s+)?(сч[её]т|заказ)|прибыти\w*\s+товар|по\s+срокам\s+успева|досыл/iu';

    /** Reply-контекст: вопрос о сроках/доставке уже размещённого заказа. */
    private const REPLY_DELIVERY_RE = '/срок\w*\s+поставк|когда\s+((можно\s+)?(будет|будем)\s+)?(поставите|отгруз\w+|отправ\w+|привез\w+|прид[её]т|приход\w+|получим|забрать|забер[её]м|планируется)/iu';
     ...
     * действительно просьба о счёте или дозаказ; вопрос «когда получим по
     * счёту № N» — постпродажа (кейсы M-2026-14700, 14012, 14183, 14235, 14236).
     *
     * @param  string  $subject
     * @param  string  $ownText  собственный текст клиента без цитаты (CitedOutboundQuoteRouter::ownBodyText)
     * @param  bool  $isReply  письмо — ответ в тред (Re:/in_reply_to)
     */
    public function wantsNewInvoiceOrOrder(string $subject, string $ownText, bool $isReply): bool
    {
        ...
        if (preg_match(self::DELIVERY_STATUS_RE, $haystack) === 1
            || ($isReply && preg_match(self::REPLY_DELIVERY_RE, $haystack) === 1)) {
            return false;
        }
        ...
        // Дозаказ / готовность оплатить / обновить КП — тоже продажа.
        if (preg_match('/\d+\s*шт|прода(ть|йте)|купить|заказать|дозаказ|готов\w*\s+опла[тч]|оплатим|к\s+оплате|обнов\w+\s+(вложенн\w+\s+)?кп/u', $haystack) === 1) {
            return true;
        }
        foreach (self::NEW_ORDER_MARKERS as $marker) {
            // «почём» — только как отдельное слово: «почему аннулировали» — не цена.
            if (in_array($marker, ['почём', 'почем'], true)) {
                if (preg_match('/(?<!\p{L})поч[её]м(?!\p{L})/u', $haystack) === 1) {
                    return true;
                }
    ...
    {
        // Единый владелец понятия «просьба о счёте» (правила и кейсы — там).
        return (new InvoiceMentionMatcher)->requestsInvoice($haystack);
    }
