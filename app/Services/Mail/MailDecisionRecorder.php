...
class MailDecisionRecorder
{
    ...
    public const STAGES = [
        ...
        'reply_awaits_parent' => ['none', 'Ответ ждёт, пока письмо-родитель станет заявкой — иначе дубль сделки'],
        'cited_quote_linked' => ['linked', 'Клиент прислал наше КП — письмо ушло в заявку, по которой оно выдано'],
        // Две записи «на всякий случай»: без них письмо, которое разбор обошёл
        // стороной или уронил, выглядит в журнале так же, как ненаписанное.
        'ingest_silent' => ['none', 'Разбор при получении не принял решения по письму'],
        'router_failed' => ['none', 'Разбор письма при получении упал с ошибкой'],
    ];
