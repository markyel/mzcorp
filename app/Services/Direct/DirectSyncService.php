...
class DirectSyncService
{
    ...
    public function decide(array $queueSkus, Collection $published, array $states, int $adsLimit): array
    {
        ...
        foreach ($published as $ad) {
            ...
            // Черновик остановить нельзя («Объявление является черновиком и не
            // может быть остановлено») — да и незачем: показов он не даёт.
            // Пройдёт модерацию — погасим следующим прогоном.
            if (! $isTarget && in_array($now, ['ON', 'OFF'], true) && $status($ad) !== 'DRAFT') {
                $suspend[] = [
                    'sku' => $ad->sku,
                    'ad_id' => (int) $ad->ad_id,
