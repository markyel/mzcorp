...
class RequestItemParsingService
{
     ...
     *   - доменные слова лифтового/эскалаторного оборудования — это деталь,
     *     даже если первое слово name'а совпадает с услугой. Кейс
     *     M-2026-2102 «Паллета траволатора Doppler» (2026-05-28): паттерн
     *     `^паллет` ловил позицию как транспортный поддон, хотя «паллета
     *     траволатора» — это плоская несущая пластина дорожки.
     */
    private function isServiceItem(?string $name): bool
    {
        ...
        // Доменный whitelist: если в name'е есть слово из лифтово-эскалаторной
        // лексики — это деталь оборудования. Зеркалит whitelist в
        // ParseItemsUnifiedPrompt; держим оба слоя синхронно.
        if (preg_match('/\b(траволатор|травалатор|травелатор|эскалатор|лифт|лебёдк|кабин|шахт|противовес|подъёмник|ступен|поручен|гребёнк|канат\s*тягов)/u', $name)) {
            return false;
        }
...
PROMPT;
    ...
    private function parseItemsFromInboundContent(
        ...
        // 3) Тело письма парсим ВСЕГДА (если есть что парсить), независимо
        // от того, нашли ли позиции в attachments. Раньше условие было
        // «парсить body только если attachments пусты» — но клиент часто
        // в теле ДОПОЛНЯЕТ позиции к PDF/КП («помимо того что в счёте,
        // добавьте позицию ABC»). Без body-парсинга такие добавления
        // терялись. Дубликаты потом снимет RequestItemPersister по
        // (article+name).
        //
        // Кейс M-2026-1493 (Зудин): прикреплены наш счёт + КП с 5
        // позициями, в теле «прошу выставить счёт на поставку позиции
        // из предыдущего счёта, добавить позицию Алюминиевая направляющая
        // поручня нижняя площадка эскалатора L=2299,5мм Н=930мм 506NCE
        // DAA402NVB5 GAA27078K14 Otis». 6-я позиция (из тела) терялась
        // потому что Vision уже извлёк 5 из PDF.
        $shouldTryText = $referenceText !== null
            || trim($subject) !== ''
            || $linkedUrlsText !== null;
        if ($shouldTryText) {
            try {
    ...
    private function parseItemsFromPhotoMarkings(
        ...
        $intro .= "═══ ТЕКСТ И ФОТО ДОПОЛНЯЮТ ДРУГ ДРУГА ═══\n"
            ...
            . "═══ КРИТИЧЕСКОЕ: ПАРАМЕТРЫ ОДНОГО ТОВАРА vs РАЗНЫЕ ПОЗИЦИИ ═══\n"
            . "Короткое перечисление в тексте через запятую/точку, где у токенов\n"
            . "НЕТ собственного qty / артикула / единицы измерения — это ПАРАМЕТРЫ\n"
            . "одной позиции, а НЕ несколько разных товаров. Не дроби такие\n"
            . "перечисления на отдельные items.\n\n"
            ...
            . "      – назначение объекта: «жилое», «жильё», «общественное»,\n"
            . "        «грузовое», «административное», «промышленное»;\n"
            ...
            . "      – исполнение: «настенное», «навесное», «врезное»,\n"
            . "        «реверсивное», «реверс», «нереверс».\n"
            . "  • Тип товара назван ОДИН раз — в теме письма / в одной фразе тела\n"
            . "    («+ фото платы», «Плата ...», «нужен блок ...») / в подписи фото.\n"
            . "  • Прикреплён ОДИН шильдик / фото-карточка одной детали.\n\n"
            . "В таких случаях → ОДНА позиция. Тип товара бери из явного упоминания\n"
            . "в письме (тема, тело, подпись фото), параметры сохраняй в name через\n"
            . "запятую/скобки И в note. Если тип товара не назван нигде явно —\n"
            . "используй обобщённый («Плата управления», «Шильдик ...») и положи\n"
            . "параметры в note.\n\n"
            . "✅ Текст: «Жилье, одиночная, нерегулируемый. + фото платы»\n"
            . "   Тема: «Запрос на плату управления УЛ»\n"
            . "   → items: [{name: \"Плата управления (жилое, одиночная работа, нерегулируемый привод)\",\n"
            . "              brand: null, article: ... (с фото), qty: 1, unit: \"шт.\",\n"
            . "              note: \"параметры: жилое, одиночная, нерегулируемый\",\n"
            . "              invoice_index: 1, image_index: 0}]\n"
            . "❌ НЕЛЬЗЯ: 3 позиции «Жилье» / «Одиночная» / «Нерегулируемый» — это\n"
            . "   характеристики ОДНОЙ платы, а не разные товары.\n\n"
            . "ЭВРИСТИКА СОМНЕНИЯ: одна короткая фраза (≤ 10 слов) без qty + фото\n"
            . "одной детали ≈ 1 позиция с параметрами. Сматчить три прилагательных\n"
            . "из такой фразы на три разные платы каталога — НИКОГДА.\n\n"
            . "Это правило ПЕРЕВЕШИВАЕТ дефолтное «количество позиций = количество\n"
            . "уникальных товаров на фото»: если 1 фото детали + 3 прилагательных в\n"
            . "тексте → 1 позиция, а не 3.\n\n"
            . "═══ ШАГ 1: ОПИСАНИЕ ФОТО ═══\n"
