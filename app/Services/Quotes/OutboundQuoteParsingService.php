...
PROMPT;
    ...
    private function verifyArticlesAgainstText(array $items, ?string $text, Request $request): array
    {
        ...
        foreach ($items as $idx => $item) {
            ...
            // Границы только по ЦИФРАМ, не по буквам: текстовый слой PDF (smalot)
            // часто склеивает артикул с соседним словом без пробела
            // («MX-fixturesM02533»), поэтому буквенная граница давала ложные
            // срабатывания. Цифровая граница ловит реальный мисрид (M00478 нет в
            // тексте, где напечатано M00878) и не флагает приклеенный, но верный
            // артикул. Правая граница (?!\d) исключает совпадение с префиксом
            // более длинного M-номера.
            $pattern = '/(?<!\d)'.preg_quote($artFolded, '/').'(?!\d)/u';
