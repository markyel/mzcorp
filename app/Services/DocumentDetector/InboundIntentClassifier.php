...
class InboundIntentClassifier
{
    ...
    public function classify(EmailMessage $message, Request $request): ?array
    {
        ...
        // отправитель, заказчика среди получателей нет — только личный CC
        // коллеги) авто-переводило «КП отправлено» → «На согласовании».
        // Помечаем intent_classified_at, чтобы догоняющий крон не перебирал.
        if (! $this->internal->affectsRequestStatus($message, $request->client_email)) {
