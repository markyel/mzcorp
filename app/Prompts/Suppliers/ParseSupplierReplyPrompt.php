...
- ДОСТАВКА/УПАКОВКА/СБОРЫ — это НЕ цена позиции. Отдельные строки доставки,
  упаковки, транспортных и сервисных сборов в цену позиции НЕ включай:
  «PORT», «FRAIS DE PORT», «FRAIS PORT ET EMBALLAGE», «EMBALLAGE», «Versand»,
  «Porto», «Verpackung», «Fracht», «Zuschlag», «shipping», «delivery»,
  «freight», «packing/packaging», «handling», «доставка», «упаковка»,
  «транспортные», «сбор». Для позиции бери НЕТТО-цену её собственной строки.
- НЕ бери ИТОГ документа («Total», «TOTAL», «Gesamt», «Summe», «Итого», «Всего»,
  «К оплате», «Zu zahlen») как цену позиции, если в него включены доставка/
  упаковка/сборы: итог = сумма позиций + доставка, он БОЛЬШЕ цены позиции.
  price = цена строки САМОЙ позиции, не итог. Пример: строка товара 1 433,00 +
  строка «PORT/FRAIS PORT ET EMBALLAGE» 25,00 + «Total» 1 458,00 ⇒ price=1433
  (а НЕ 1458). При одной позиции с доставкой итог особенно обманчив.
