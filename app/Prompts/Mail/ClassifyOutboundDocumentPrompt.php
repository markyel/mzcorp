...
3. clarification — менеджер ЗАДАЁТ КЛИЕНТУ ВОПРОС / просит у него данные и
   ЖДЁТ ОТВЕТА КЛИЕНТА, чтобы продолжить (ход переходит к клиенту). Нет
   вложения с КП/счётом.
   Сигналы (обращение к клиенту в повелительном/вопросительном):
   • «уточните», «пришлите», «подскажите», «не хватает информации», «какая
     модель?», «для подготовки КП пришлите…», «фото маркировки».
   • Вложений нет ИЛИ только наши уточняющие документы (анкета, опросник).
   ВАЖНО — это НЕ clarification (ход остаётся за НАМИ → type=other):
   • Менеджер обещает САМ что-то сделать: «уточню стоимость/цену/наличие»,
     «позже/завтра направлю КП», «подготовлю предложение», «вернусь с ценой/
     сроком», «проверю и сообщю». Здесь мы НЕ ждём клиента — мы сами работаем
     над КП. Глагол в 1-м лице будущего («уточню», «направлю», «подготовлю»)
     ≠ просьба к клиенту («уточните», «пришлите»).
...
5. other — обычная переписка, не вызывающая авто-перехода статуса.
   ...
   • Менеджер обещает САМ прислать КП/уточнить цену позже: «уточню стоимость,
     позже направлю КП», «подготовлю предложение», «вернусь с ценой» — ход за
     нами, статус не меняем (это НЕ clarification).
...
• clarification ТОЛЬКО когда менеджер ждёт ОТВЕТА КЛИЕНТА (вопрос/просьба к
  клиенту). Обещание менеджера самому уточнить цену / прислать КП позже — это
  other (ход за нами, не ждём клиента). «уточню» (я) ≠ «уточните» (вы).
