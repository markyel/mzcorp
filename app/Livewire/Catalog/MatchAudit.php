...
class MatchAudit extends Component
{
    ...
    private function baseQuery()
    {
        $q = DB::table('outbound_quote_items as oqi')
            ...
            // ЛЮБОЙ строкой КП этой же ЗАЯВКИ: значит система сматчила верно, а
            // «диф» — от мис-привязки другой строки КП к этой же позиции (одна
            // позиция заявки ← много строк КП). Кейс M-2026-9753 поз.1: система
            // M00021 подтверждена строкой КП M00021, а этажные кнопки M03135+ —
            // чужие строки. Уровень ЗАЯВКИ (а не одного oq), т.к. КП часто
            // распарсен несколько раз (несколько outbound_quotes) и верный парс
            // лежит в соседнем — кейс M-2026-9728 поз.3 (парсер спутал артикул
            // «Троса» на M13668=«КВШ», а верный M13658 — в другом oq того же КП).
            ->whereNotExists(function ($sub) {
                $sub->selectRaw('1')->from('outbound_quote_items as o2')
                    ->join('outbound_quotes as oq2', 'oq2.id', '=', 'o2.outbound_quote_id')
                    ->whereColumn('oq2.request_id', 'oq.request_id')
