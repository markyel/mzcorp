    ...
    public static function schema(): array
    {
        return [
            ...
            'assignment.mode' => [
                ...
                'label' => 'Правило раздачи заявок',
                'help' => 'Умный режим: заявка сначала ищет «своего» менеджера (тот же товар, тот же клиент, письмо в личный ящик), и только потом идёт в балансировку по нагрузке и скорости закрытия. Пропорциональный: общий поток с info раздаётся по очереди, учитывается только процент нагрузки менеджера из его карточки — привязки по товару, клиенту и тексту не действуют. Единственное исключение: заявка, полностью совпавшая по составу с открытой заявкой другого покупателя за последнюю неделю, уходит тому же менеджеру и идёт ему в зачёт. Письмо, пришедшее прямо в личный ящик менеджера, закрепляется за ним в обоих режимах. Доступность тоже учитывается в обоих: отсутствующему новые заявки не приходят, письма с его личного ящика уходят в общую раздачу, а открытые заявки делегируются коллегам пропорционально их проценту.',
                'type' => AppSetting::TYPE_STRING,
                'default' => \App\Services\Request\AssignmentService::MODE_SMART,
                'options' => [
                    \App\Services\Request\AssignmentService::MODE_SMART => 'умный (sticky + балансировка)',
                    \App\Services\Request\AssignmentService::MODE_PROPORTIONAL => 'пропорциональный (по проценту нагрузки)',
                ],
            ],
            ...
            'auto_close.under_review_days' => [
                'group' => 'Авто-закрытие по молчанию',
                'label' => 'Молчание на согласовании → закрыть (раб. дней)',
                'help' => 'Если заявка в статусе «На согласовании» (клиент сказал, что рассматривает КП) и клиент молчит дольше N рабочих дней — авто-закрывается как «Клиент молчит после КП». Не сработает, если клиент писал в последние N раб. дней. Отдельно от «после КП», т.к. согласованию обычно дают больше времени. 0 = выключить правило.',
                'type' => AppSetting::TYPE_INT,
                'default' => 0,
                'step' => 1,
                'min' => 0,
            ...
            'auto_close.reminder_grace_days' => [
                'group' => 'Авто-закрытие по молчанию',
                'label' => 'Пауза после напоминания клиенту (раб. дней)',
                'help' => 'После того как мы САМИ отправили клиенту напоминание (по уточнению, КП или счёту), авто-закрытие откладывается на N рабочих дней от даты напоминания — чтобы человек успел ответить. Иначе заявка закрывается прямо перед его ответом. 0 = выключить паузу.',
                'type' => AppSetting::TYPE_INT,
                'default' => 3,
                'step' => 1,
                'min' => 0,
            ...
            // ─── Реклама в письмах ────────────────────────────────────────
            'marketing.block_position' => [
                'group' => 'Реклама в письмах',
                'label' => 'Положение рекламного блока',
                'help' => 'Где вставлять блок в письмах клиентам: над подписью менеджера (сразу после текста письма) или под ней. Действует на все письма через MyLift — ручные ответы и авто-уведомления. Сами блоки — в разделе «Реклама».',
                'type' => AppSetting::TYPE_STRING,
                'default' => (string) config('services.marketing.block_position', 'below'),
                'options' => ['below' => 'под подписью', 'above' => 'над подписью'],
            ],
        ];
    }
