    ...
    public static function schema(): array
    {
        return [
            ...
            'assignment.mode' => [
                ...
                'help' => 'Умный режим: заявка сначала ищет «своего» менеджера (тот же товар, тот же клиент, письмо в личный ящик), и только потом идёт в балансировку по нагрузке и скорости закрытия. Пропорциональный: никаких привязок — заявки раздаются по очереди, учитывается только процент нагрузки менеджера из его карточки. Доступность менеджера учитывается в обоих: отсутствующему новые заявки не приходят, а его текущие делегируются коллегам пропорционально их проценту.',
                'type' => AppSetting::TYPE_STRING,
