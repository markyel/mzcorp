...
class Index extends Component
{
    ...
    public static function schema(): array
    {
        return [
            ...
            // Крон requests:auto-close-inactive (раз в сутки). Календарные дни.
            // 0 = правило выключено. Закрытие — closed_lost с соответствующей
            // причиной; восстанавливается вручную кнопкой «↻ Реанимировать».
            'auto_close.clarification_days' => [
                'group' => 'Авто-закрытие по молчанию',
                'label' => 'Молчание после уточнения → закрыть (дней)',
                'help' => 'Если клиент не ответил на уточняющий вопрос дольше N календарных дней — заявка авто-закрывается как «Клиент молчит после уточнения». 0 = выключить правило.',
                'type' => AppSetting::TYPE_INT,
                'default' => 4,
                'step' => 1,
                'min' => 0,
            ...
            'auto_close.quote_days' => [
                'group' => 'Авто-закрытие по молчанию',
                'label' => 'Молчание после КП → закрыть (дней)',
                'help' => 'Если клиент не отреагировал на отправленное КП дольше N календарных дней — заявка авто-закрывается как «Клиент молчит после КП». 0 = выключить правило.',
                'type' => AppSetting::TYPE_INT,
                'default' => 5,
                'step' => 1,
                'min' => 0,
            ...
            'auto_close.invoice_days' => [
                'group' => 'Авто-закрытие по молчанию',
                'label' => 'Счёт не оплачен → закрыть (дней от выставления)',
                'help' => 'Если счёт не оплачен дольше N календарных дней с даты выставления — заявка авто-закрывается как «Счёт не оплачен в срок». 0 = выключить правило.',
                'type' => AppSetting::TYPE_INT,
                'default' => 5,
                'step' => 1,
                'min' => 0,
