...
enum ClientNotificationType: string
{
    ...
    /**
     * «Оживляющее» письмо по проигранной заявке. Cron. Условия: Request
     * closed_lost закрыта за период (default 14 дн.), по ней было отправлено
     * КП, и по позициям заявки цена в каталоге УПАЛА сильнее порога (default
     * 15%) уже ПОСЛЕ отправки КП. Предлагаем подготовить актуальное КП. На
     * положительный ответ клиента заявку реанимируем. Одно письмо на КП.
     */
    case RevivalOffer = 'revival_offer';
    ...
    public function label(): string
    {
        return match ($this) {
            ...
            self::RevivalOffer => 'Оживление по падению цены',
        };
    }
    ...
    public function description(): string
    {
        return match ($this) {
            ...
            self::QuoteFollowupReminder => 'Если после отправки коммерческого предложения клиент не отреагировал в течение заданного срока (порог «КП отправлено — первый нудж»). Также охватывает статус «На согласовании» — когда клиент подтвердил, что рассматривает КП, но затем замолчал (свой порог «На согласовании»).',
            ...
            self::RevivalOffer => 'Для проигранных заявок (закрытых за последние N дней), по которым было КП, и цена на позиции с тех пор упала сильнее порога. Предлагаем клиенту подготовить актуальное КП. Если клиент ответит согласием — заявка автоматически оживляется. Одно письмо на каждое КП.',
        };
    }
    ...
    public function placeholders(): array
    {
        ...
        $specific = match ($this) {
            ...
            self::RevivalOffer => [
                'quote_number' => 'Номер ранее отправленного КП (напр. МЗ-357197)',
                'quote_date' => 'Дата отправки КП (DD.MM.YYYY)',
                'dropped_summary' => 'Краткий список позиций, на которые упала цена',
                'reply_keyword' => 'Фраза, которую просим написать в ответ (по умолчанию «прислать новое КП»)',
            ],
        };
