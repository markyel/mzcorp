    ...
    public function allowedTransitions(): array
    {
        return match ($this) {
            ...
            self::AwaitingInvoice => [
                ...
                // Откат на «КП отправлено» при ВЫДАЧЕ НОВОГО КП: если после
                // «ждёт счёт» изменилось наличие/цена и менеджер выслал новое
                // КП, оффер поменялся — клиент должен пересмотреть его до счёта
                // (кейс M-2026-6286). Обычно движемся только вперёд, но новое
                // КП — осознанное действие менеджера, и OutboundQuotationFull-
                // детектор авто-откатывает сюда. Также ручной escape-hatch.
                self::Quoted,
                self::Invoiced,
