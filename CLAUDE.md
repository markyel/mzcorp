...
8. **Не ставь IMAP-флаг `\Seen` при чтении/разборе писем.** Чтение в READ-ONLY режиме (FT_PEEK), без побочных эффектов на флаги.
   ...
   **Исключение 2 (2026-09-08):** владелец ЛИЧНОГО ящика читает/помечает письмо в разделе «Почта» → `ImapSeenSyncService` ставит/снимает `\Seen` на сервере, и наоборот подтягивает `\Seen` с сервера (pull). Это явное действие пользователя над своим ящиком, как в обычном почтовом клиенте; парсер/синк по-прежнему читают без побочных эффектов, чужие ящики (директор, РОП) IMAP не трогают.
